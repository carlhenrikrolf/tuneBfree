...
static volatile enum { Run, Exit } client_state = Run;
...
static float *delay_port(struct DelayBuffer *dly, uint32_t n_samples, float *in)
{
    uint32_t pos = 0;
...
static int process(jack_nframes_t nframes, void *arg)
{
    ...
    const bool transport_changed =
        (rolling != j_transport.rolling || pos.frame != j_transport.position ||
    ...
    for (uint32_t i = 0; i < inst->nports_audio_out; i++)
    {
        plugin_dsp->connect_port(plugin_instance, portmap_a_out[i],
    ...
    for (uint32_t i = 0; i < inst->nports_audio_in; i++)
    {
        ...
        plugin_dsp->connect_port(plugin_instance, portmap_a_in[i],
                                 delay_port(delayline[i], nframes,
                                            (float *)jack_port_get_buffer(input_port[i], nframes)));
    }
    ...
    if (gui_instance)
    {
        for (uint32_t p = 0; p < inst->nports_ctrl; p++)
        {
            ...
            if (plugin_ports_pre[p] != plugin_ports_post[p])
            {
                ...
                {
                    jack_ringbuffer_write(rb_ctrl_to_ui, (char *)&portmap_rctl[p],
                                          sizeof(uint32_t));
                    jack_ringbuffer_write(rb_ctrl_to_ui, (char *)&plugin_ports_pre[p],
                                          sizeof(float));
                }
            }
        }
    ...
    {
        if (gui_instance && jack_ringbuffer_write_space(rb_atom_to_ui) >=
                                atom_out->atom.size + 2 * sizeof(LV2_Atom))
        {
        ...
        {
            void *buf = jack_port_get_buffer(midi_out, nframes);
            LV2_Atom_Event const *ev =
                (LV2_Atom_Event const *)((&(atom_out)->body) + 1); // lv2_atom_sequence_begin
            while ((const uint8_t *)ev <
                   ((const uint8_t *)&(atom_out)->body + (atom_out)->atom.size))
            {
                if (ev->body.type == uri_midi_MidiEvent)
                {
                    jack_midi_event_write(buf, ev->time.frames, (const uint8_t *)(ev + 1),
                                          ev->body.size);
                }
...
static int jack_graph_order_cb(void *arg)
{
    ...
    for (uint32_t i = 0; i < inst->nports_audio_in; i++)
    {
        jack_port_get_latency_range(input_port[i], JackCaptureLatency,
                                    &(delayline[i]->port_latency));
...
static void jack_freewheel_cb(int onoff, void *arg) { _freewheeling = onoff; }
...
static int jack_portsetup(void)
{
    ...
    if (inst->nports_audio_in > 0)
    {
        ...
        delayline =
            (struct DelayBuffer **)calloc(inst->nports_audio_in, sizeof(struct DelayBuffer *));
    }

    for (uint32_t i = 0; i < inst->nports_audio_in; i++)
    {
        if ((input_port[i] = jack_port_register(j_client, inst->ports[portmap_a_in[i]].name,
    ...
    for (uint32_t i = 0; i < inst->nports_audio_out; i++)
    {
        if ((output_port[i] = jack_port_register(j_client, inst->ports[portmap_a_out[i]].name,
    ...
    {
        if ((midi_in = jack_port_register(j_client, inst->ports[portmap_atom_from_ui].name,
    ...
    if (inst->nports_midi_out)
    {
        if ((midi_out = jack_port_register(j_client, inst->ports[portmap_atom_to_ui].name,
...
{
    if (which & 1)
    { // connect audio input(s)
        const char **ports = jack_get_ports(j_client, NULL, JACK_DEFAULT_AUDIO_TYPE,
    ...
    if ((which & 4) && midi_in)
    { // midi in
        const char **ports = jack_get_ports(j_client, NULL, JACK_DEFAULT_MIDI_TYPE,
                                            JackPortIsOutput | JackPortIsPhysical);
        if (ports && ports[0])
        {
...
static void write_function(LV2UI_Controller controller, uint32_t port_index, uint32_t buffer_size,
...
static LV2_Worker_Status lv2_worker_respond(LV2_Worker_Respond_Handle unused, uint32_t size,
                                            const void *data)
{
...
static LV2_Worker_Status lv2_worker_schedule(LV2_Worker_Schedule_Handle unused, uint32_t size,
                                             const void *data)
{
    if (_freewheeling)
    {
...
static int oscb_noteon(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg,
                       void *user_data)
{
    MIDI_Q3(0x90);
    return 0;
...
static int oscb_noteoff(const char *path, const char *types, lo_arg **argv, int argc,
...
static int oscb_cc(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg,
                   void *user_data)
{
    MIDI_Q3(0xb0);
    return 0;
...
static int oscb_pc(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg,
                   void *user_data)
{
    osc_midi_event_t ev;
    ev.size = 2;
...
static int oscb_rawmidi(const char *path, const char *types, lo_arg **argv, int argc,
                        lo_message msg, void *user_data)
{
    osc_midi_event_t ev;
    ev.size = 3;
...
static int oscb_parameter(const char *path, const char *types, lo_arg **argv, int argc,
                          lo_message msg, void *user_data)
{
...
static int start_osc_server(int osc_port)
{
    ...
    for (size_t i = 0; i < sizeof(OSCC) / sizeof(struct osc_command); ++i)
    {
        lo_server_thread_add_method(osc_server, OSCC[i].path, OSCC[i].typespec, OSCC[i].handler,
                                    NULL);
    }
...
static void run_one(LV2_Atom_Sequence *data)
{
    ...
    while (jack_ringbuffer_read_space(rb_atom_to_ui) > sizeof(LV2_Atom))
    {
        ...
        LV2_Atom_Event const *ev =
            (LV2_Atom_Event const *)((&(data)->body) + 1); // lv2_atom_sequence_begin
        while ((const uint8_t *)ev < ((const uint8_t *)&(data)->body + (data)->atom.size))
        {
            plugin_gui->port_event(gui_instance, portmap_atom_to_ui, ev->body.size,
                                   uri_atom_EventTransfer, &ev->body);
            ev = (LV2_Atom_Event const *)/* lv2_atom_sequence_next() */
                ((const uint8_t *)ev + sizeof(LV2_Atom_Event) + ((ev->body.size + 7) & ~7));
        }
    }
...
static void on_external_ui_closed(void *controller) { catchsig(0); }
...
static void print_usage(void)
{
    ...
    printf("\nUsage:\n"
           ...
           " Click+Drag     left/down: decrease, right/up: increase value. Hold "
           "the Ctrl key to "
           "increase sensitivity.\n"
           " Shift+Click    reset to default value\n"
           " Scroll-wheel    up/down by 1 step (smallest possible adjustment for "
           "given setting). "
           "Rapid continuous scrolling increases the step-size.\n"
           "The application can be closed by sending a SIGTERM (CTRL+C) on the "
           "command-line of by "
           "closing the window.\n");
...
#endif
           " -O <port>, --osc <port>   Listen for OSC messages on the given UDP "
           "port\n"
...
int main(int argc, char **argv)
{
    ...
    const struct option long_options[] = {
        {"help", no_argument, 0, 'h'},       {"jack-name", required_argument, 0, 'j'},
        {"list", no_argument, 0, 'l'},       {"nogui", no_argument, 0, 'G'},
        {"osc", required_argument, 0, 'O'},  {"osc-doc", no_argument, 0, 0x100},
        {"port", required_argument, 0, 'p'}, {"portlist", no_argument, 0, 'P'},
...
#ifdef _WIN32
        MessageBox(NULL,
                   TEXT("JACK is not available.\n"
                        "You must have the JACK Audio Connection Kit installed to "
                        "use the tools. "
                        "Please see http://jackaudio.org/ and "
                        "http://jackaudio.org/faq/jack_on_windows.html"),
                   TEXT("Error"), MB_ICONERROR | MB_OK);
#elif __APPLE__
        rtk_osx_api_err("JACK is not available.\n"
                        "You must have the JACK Audio Connection Kit installed to "
                        "use the tools. "
    ...
    const LV2_Feature *features[] = {&map_feature, &unmap_feature, &schedule_feature, NULL};
    ...
    const LV2_Feature *ui_features[] = {&map_feature,         &unmap_feature,
                                        &instance_feature,    &external_lv_feature,
                                        &external_kx_feature, NULL};
    ...
    rb_ctrl_to_ui =
        jack_ringbuffer_create((UPDATE_FREQ_RATIO)*inst->nports_ctrl * 2 * sizeof(float));
    rb_ctrl_from_ui =
        jack_ringbuffer_create((UPDATE_FREQ_RATIO)*inst->nports_ctrl * 2 * sizeof(float));
...
#ifdef _WIN32
        MessageBox(NULL,
                   TEXT("Cannot connect to JACK.\n"
                        "Please start the JACK Server first."),
                   TEXT("Error"), MB_ICONERROR | MB_OK);
#elif __APPLE__
        rtk_osx_api_err("Cannot connect to JACK.\n"
                        "Please start the JACK Server first.");
#else
        (void)system("xmessage -button ok -center \"Cannot connect to "
                     "JACK.\nPlease start the JACK "
                     "Server first.\" &");
#endif
        rv |= 4;
        goto out;
    ...
    if (inst->nports_atom_out > 0 || inst->nports_atom_in > 0 || inst->nports_midi_in > 0 ||
        inst->nports_midi_out > 0)
    {
    ...
    for (uint32_t i = 0; i < n_pval; ++i)
    {
        ...
        if (inst->ports[port_index].val_min < inst->ports[port_index].val_max)
        {
            if (pval[i].value < inst->ports[port_index].val_min ||
                pval[i].value > inst->ports[port_index].val_max)
            {
                fprintf(stderr, "Value for port %d out of bounds %f <= %f <= %f\n",
                        pval[i].port_idx, inst->ports[port_index].val_min, pval[i].value,
                        inst->ports[port_index].val_max);
                continue;
            }
        }

        // fprintf (stdout, "Port: %d,  %d -> %f\n", pval[i].port_idx, port_index,
        // pval[i].value);
    ...
    if (plugin_gui && !headless)
    {
        ...
        gui_instance = plugin_gui->instantiate(plugin_gui, plugin_dsp->URI, NULL, &write_function,
                                               controller, (void **)&extui, ui_features);
    }
    ...
    else
    {
        ...
        LV2_Atom_Sequence *data =
            (LV2_Atom_Sequence *)malloc(inst->min_atom_bufsiz * sizeof(uint8_t));
        CFRunLoopRef runLoop = CFRunLoopGetCurrent();
        CFRunLoopTimerContext context = {0, data, NULL, NULL, NULL};
        CFRunLoopTimerRef timer = CFRunLoopTimerCreate(kCFAllocatorDefault, 0, 1.0 / UI_UPDATE_FPS,
                                                       0, 0, &osx_loop, &context);
