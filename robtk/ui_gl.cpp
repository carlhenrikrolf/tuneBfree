...
// #define USE_GUI_THREAD // use own thread (not idle callback), should be
// preferred even w/idle availale on most platforms #define THREADSYNC     //
// wake up GUI thread on port-event
...
static void opengl_draw(int width, int height, unsigned char *surf_data, unsigned int texture_id)
{
    ...
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8, width, height,
                 /*border*/ 0, GL_BGRA, GL_UNSIGNED_BYTE, surf_data);
...
static void opengl_reallocate_texture(int width, int height, unsigned int *texture_id)
{
    ...
    glBindTexture(GL_TEXTURE_RECTANGLE_ARB, *texture_id);
    glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA8, width, height, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                 NULL);
    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);
}

static cairo_t *opengl_create_cairo_t(int width, int height, cairo_surface_t **surface,
                                      unsigned char **buffer)
{
    ...
    *surface = cairo_image_surface_create_for_data(*buffer, CAIRO_FORMAT_ARGB32, width, height,
                                                   bpp * width);
...
static void lc_expose(GLrobtkLV2UI *self)
{
    ...
    if (self->gpg_shade < .6)
        self->gpg_shade += .001;
    cairo_fill(self->cr);
    write_text_full(self->cr, "Unregistered Version\nhttp://x42-plugins.com", xfont,
                    self->width * .5, self->height * .5, self->width < 200 ? M_PI * -.5 : 0, -2,
                    c_wht);
...
static void cairo_expose(GLrobtkLV2UI *self)
{
        ...
        if (fast_track_cnt > 0)
        {
                ...
                a.a.y + a.rw->trel.y + a.a.height <= fast_track_area.y + fast_track_area.height)
            {
                ...
                fprintf(stderr, " skip fast-track event #%d (%.1f x %.1f + %.1f + %.1f)\n",
                        fast_track_cnt, a.a.width, a.a.height, a.a.x + a.rw->trel.x,
                        a.a.y + a.rw->trel.y);
#endif
                continue;
            }
...
#ifdef DEBUG_EXPOSURE
    fprintf(stderr, "XPS %.1f+%.1f  %.1fx%.1f\n", self->expose_area.x, self->expose_area.y,
            self->expose_area.width, self->expose_area.height);
#endif
    ...
    expose_area.width = MIN(area.x + area.width, self->tl->area.x + self->tl->area.width) -
                        MAX(area.x, self->tl->area.x);
    expose_area.height = MIN(area.y + area.height, self->tl->area.y + self->tl->area.height) -
                         MAX(area.y, self->tl->area.y);
...
#define XPS_NO_DRAW                                                                                \
    {                                                                                              \
        fprintf(stderr, " !!! OUTSIDE DRAW %.1fx%.1f %.1f+%.1f %.1fx%.1f\n", area.x, area.y,       \
                self->tl->area.x, self->tl->area.y, self->tl->area.width, self->tl->area.height);  \
        return;                                                                                    \
    }

#if 1
    if (area.x > self->tl->area.x + self->tl->area.width)
        XPS_NO_DRAW
    if (area.y > self->tl->area.y + self->tl->area.height)
        XPS_NO_DRAW
    if (area.x < self->tl->area.x)
        XPS_NO_DRAW
    if (area.y < self->tl->area.y)
        XPS_NO_DRAW
#endif
...
{
    GLrobtkLV2UI *self = (GLrobtkLV2UI *)robwidget_get_toplevel_handle(rw);
    if (!self || !self->view)
    {
        rw->redraw_pending = true;
        return;
    ...
    if (y < 0)
        y = 0;
    if (x + width > rw->area.width)
        width = rw->area.width - x;
    if (y + height > rw->area.height)
        height = rw->area.height - y;
    ...
    else
    {
        ...
        r.width = width;
        r.height = height;
        rect_combine((cairo_rectangle_t *)&self->expose_area, &r,
                     (cairo_rectangle_t *)&self->expose_area);
    }
    puglPostRedisplay(self->view);
}
...
static void queue_tiny_rect(RobWidget *rw, cairo_rectangle_t *a)
{
    ...
    GLrobtkLV2UI *self = (GLrobtkLV2UI *)robwidget_get_toplevel_handle(rw); // XXX cache ?!
    if (!self || !self->view)
    {
        rw->redraw_pending = true;
        return;
...
static void robwidget_layout(GLrobtkLV2UI *const self, bool setsize, bool init)
{
    ...
    printf("robwidget_layout(%s) setsize:%s init:%s\n", ROBWIDGET_NAME(self->tl),
           setsize ? "true" : "false", init ? "true" : "false");
#endif
...
static void set_toplevel_expose_overlay(RobWidget *rw,
                                        void (*expose_event)(struct _robwidget *, cairo_t *,
                                                             cairo_rectangle_t *))
{
    GLrobtkLV2UI *const self = (GLrobtkLV2UI *)robwidget_get_toplevel_handle(rw);
...
static void robtk_expose_ui_scale(RobWidget *handle, cairo_t *cr, cairo_rectangle_t *ev)
{
    ...
    static const char scales[8][8] = {"100%", "110%", "115%", "120%",
                                      "125%", "150%", "175%", "200%"};
    ...
    for (int y = 0; y < nbtn_row; ++y)
    {
        for (int x = 0; x < nbtn_col; ++x)
        {
            ...
            write_text_full(cr, scales[pos], font, floor(x0 + bt_w * .5), floor(y0 + bt_h * .5), 0,
                            2, c_wht);
        }
    }
    pango_font_description_free(font);
...
static void robwidget_toplevel_enable_scaling(RobWidget *rw, void (*cb)(RobWidget *w, void *handle),
                                              void *handle)
{
    assert(rw->parent == rw);
...
static void reallocate_canvas(GLrobtkLV2UI *self)
{
    ...
    self->cr = opengl_create_cairo_t(hw_scale * self->width, hw_scale * self->height,
                                     &self->surface, &self->surf_data);

#if __BIG_ENDIAN__
    self->surf_data_be =
        (unsigned char *)calloc(4 * self->width * self->height, sizeof(unsigned char));
#endif
...
static void onRealReshape(PuglView *view, int width, int height)
{
    ...
    case LVGL_ZOOM_TO_ASPECT:
        ...
        else
        {
            ...
            glViewport(self->xoff, self->yoff, self->width / self->xyscale,
                       self->height / self->xyscale);
        }
        break;
    case LVGL_CENTER:
    {
...
static void onResize(PuglView *view, int *width, int *height, int *set_hints)
{
    ...
    *height = self->height;
    ...
    {
        gtk_window_resize(GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(self->resize->handle))), w,
                          h);
    }
#endif
...
static void onMotion(PuglView *view, int x, int y)
{
    ...
    if (self->mousefocus || self->tl->block_events)
        return;
#endif
...
static void onMouse(PuglView *view, int button, bool press, int x, int y)
{
    ...
    {
        if (x > self->tl->area.x + self->tl->area.width)
            return;
        if (y > self->tl->area.y + self->tl->area.height)
            return;
        if (x < self->tl->area.x)
            return;
        if (y < self->tl->area.y)
...
static void onScroll(PuglView *view, int x, int y, float dx, float dy)
{
    ...
    if (self->tl->mousescroll)
        self->tl->mousescroll(self->tl, &event);
}
...
static int pugl_init(GLrobtkLV2UI *self)
{
    ...
    // Set up GL UI
    self->view = puglCreate(self->extui ? (PuglNativeWindow)NULL : self->parent,
                            self->extui ? self->extui->plugin_human_id : RTK_URI, self->width,
                            self->height, dflw, dflh,
...
static int idle(LV2UI_Handle handle)
{
    ...
    if (self->do_the_funky_resize && self->resize)
    {
        ...
        if (gtk_widget_get_toplevel(GTK_WIDGET(self->resize->handle)))
        {
            gtk_window_resize(GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(self->resize->handle))),
                              w, h);
        }
#endif
...
static LV2UI_Handle gl_instantiate(const LV2UI_Descriptor *descriptor, const char *plugin_uri,
                                   ...
                                   const LV2_Feature *const *features)
{
    ...
    if (options && map)
    {
        ...
        LV2_URID transient_for =
            map->map(map->handle, "http://kxstudio.sf.net/ns/lv2ext/props#TransientWindowId");
#ifdef RTK_USE_HOST_COLORS
        LV2_URID atom_Int = map->map(map->handle, LV2_ATOM__Int);
        LV2_URID color_fg =
            map->map(map->handle, "http://lv2plug.in/ns/extensions/ui#foregroundColor");
        LV2_URID color_bg =
            map->map(map->handle, "http://lv2plug.in/ns/extensions/ui#backgroundColor");
#endif

        for (const LV2_Options_Option *o = options; o->key; ++o)
        {
            if (o->context == LV2_OPTIONS_INSTANCE && o->key == transient_for &&
                o->type == atom_Long)
            {
                self->transient_id = *(const unsigned long *)o->value;
            }
#ifdef RTK_USE_HOST_COLORS
            // TODO: consider adding a host bg colored border when the plugin does not
            // use RTK_USE_HOST_COLORS
            else if (o->context == LV2_OPTIONS_INSTANCE && o->key == color_fg &&
                     o->type == atom_Int)
            {
                set_host_color(0, *(const uint32_t *)o->value);
            }
            else if (o->context == LV2_OPTIONS_INSTANCE && o->key == color_bg &&
                     o->type == atom_Int)
            {
...
#include "gpg_check.cpp"
    ...
    self->tl = NULL;
    self->ui = instantiate(self, descriptor, plugin_uri, bundle_path, write_function, controller,
                           &self->tl, features);
...
static void gl_port_event(LV2UI_Handle handle, uint32_t port_index, uint32_t buffer_size,
                          uint32_t format, const void *buffer)
{
...
static const void *gl_extension_data(const char *uri)
{
#ifdef HAVE_IDLE_IFACE
...
static const LV2UI_Descriptor gl_descriptor = {RTK_URI RTK_GUI "_gl", gl_instantiate, gl_cleanup,
                                               gl_port_event, gl_extension_data};

#ifdef RTK_DESCRIPTOR // standalone lv2

const LV2UI_Descriptor *RTK_DESCRIPTOR(uint32_t index) { return &gl_descriptor; }
...
#if (defined _WIN32 && defined RTK_STATIC_INIT)
static void __attribute__((constructor)) x42_init() { pthread_win32_process_attach_np(); }
