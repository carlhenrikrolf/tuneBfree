...
static inline float db_to_coefficient(const float d) { return powf(10.0f, 0.05f * d); }

#define SILENT 128 // cycles of 64
#define FADED 96   // cycles of 64

static bool faded(B3W *b3w) { return b3w->fade >= FADED; }
...
static void process(B3W *b3w, uint32_t n_samples, float const *const in, float *outL, float *outR)
{
    ...
    whirlProc2(b3w->whirl, in, NULL, NULL, horn_left, horn_right, drum_left, drum_right, n_samples);
    ...
    {
        outL[i] =
            horn_left[i] * hll + horn_right[i] * hlr + drum_left[i] * dll + drum_right[i] * dlr;
        outR[i] =
            horn_left[i] * hrl + horn_right[i] * hrr + drum_left[i] * drl + drum_right[i] * drr;
    }
}
...
static const LV2_Descriptor descriptor = {B3W_URI, instantiate, connect_port, NULL,
                                          run,     NULL,        cleanup,      extension_data};

static const LV2_Descriptor descriptorExt = {B3W_URI_EXT, instantiate, connect_port,  NULL, run,
                                             NULL,        cleanup,     extension_data};

static const LV2_Descriptor descriptorMOD = {B3W_URI_MOD, instantiate, connect_port,  NULL, run,
                                             NULL,        cleanup,     extension_data};
