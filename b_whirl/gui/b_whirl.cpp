...
static float get_eq_response(FilterSection *flt, const float freq)
{
    ...
    const float rv = 20.f * log10f(sqrtf((SQUARE(A) + SQUARE(B)) * (SQUARE(C) + SQUARE(D))) /
                                   (SQUARE(C) + SQUARE(D)));
    return MAX(-100, rv);
}
...
#define FREQLINE(FQ)                                                                               \
    {                                                                                              \
        ...
        cairo_move_to(cr, xf, 2);                                                                  \
        cairo_line_to(cr, xf, h - 2);                                                              \
        cairo_stroke(cr);                                                                          \
    }
...
static void m0_size_request(RobWidget *handle, int *w, int *h)
{
    *w = 60;
    *h = 50;
}
...
static void m0_filter_touch(WhirlUI *ui, int f, bool touch)
{
    if (!ui->touch)
        return;
    if (f < 0 || f > 2)
...
static void m0_size_allocate(RobWidget *rw, int w, int h)
{
    WhirlUI *ui = (WhirlUI *)GET_HANDLE(rw);
...
{
    if (ui->eq_ctrl[f].x0 < 0 || ui->eq_ctrl[f].y0 < 0)
        return false;
...
static void horn_size_request(RobWidget *handle, int *w, int *h)
{
    *w = 60;
    *h = 40;
}

static void drum_size_request(RobWidget *handle, int *w, int *h)
{
    *w = 60;
    *h = 120;
}

static void m1_size_allocate(RobWidget *rw, int w, int h)
{
    WhirlUI *ui = (WhirlUI *)GET_HANDLE(rw);
    robwidget_set_size(rw, w, h);
    for (int i = 0; i < 4; ++i)
    {
        if (ui->hornp[i])
        {
            cairo_pattern_destroy(ui->hornp[i]);
            ui->hornp[i] = NULL;
        }
    }
}

static void create_horn_patterns(WhirlUI *ui, float x, float y)
{
    ...
    cairo_pattern_add_color_stop_rgb(ui->hornp[2], 0.00, .4750, .4750,
                                     .4750); // a=0.
    cairo_pattern_add_color_stop_rgb(ui->hornp[2], 0.10, .3925, .3925,
                                     .3925); // a=.3  c=.2
    cairo_pattern_add_color_stop_rgb(ui->hornp[2], 0.66, .5125, .5125,
                                     .5125); // a=.5  c=.9
    cairo_pattern_add_color_stop_rgb(ui->hornp[2], 0.90, .3925, .3925,
                                     .3925); // a=.3  c=.2
    cairo_pattern_add_color_stop_rgb(ui->hornp[2], 1.00, .4750, .4750,
                                     .4750); // a=0.

    // base g30
    ui->hornp[3] = cairo_pattern_create_linear(-x * .5, 0, x * .5, 0);
    cairo_pattern_add_color_stop_rgb(ui->hornp[3], 0.00, .30, .30, .30); // a=0.
    cairo_pattern_add_color_stop_rgb(ui->hornp[3], 0.10, .27, .27,
                                     .27); // a=.3  c=.2
    cairo_pattern_add_color_stop_rgb(ui->hornp[3], 0.66, .60, .60,
                                     .60); // a=.5  c=.9
    cairo_pattern_add_color_stop_rgb(ui->hornp[3], 0.90, .27, .27,
                                     .27);                               // a=.3  c=.2
...
#define DRAW_HORN(XX, YY, CMP, CLR, BASE)                                                          \
    ...
    cairo_arc(cr, 0, 0, rd, 0, 2. * M_PI);                                                         \
                                                                                                   \
    cairo_matrix_init_translate(&matrix, 0, CMP(ang < M_PI) ? -y0 - (YY y) : y0 + (YY y));         \
    cairo_pattern_set_matrix(ui->hornp[CLR], &matrix);                                             \
    cairo_set_source(cr, ui->hornp[CLR]);                                                          \
...
static bool cb_dial_brk0(RobWidget *w, void *handle)
{
    ...
    if (!robtk_dial_get_state(ui->s_brakepos[0]))
    {
        val = 0;
    }
...
static bool cb_dial_brk1(RobWidget *w, void *handle)
{
    ...
    if (!robtk_dial_get_state(ui->s_brakepos[1]))
    {
        val = 0;
    }
...
static bool cb_linked(RobWidget *w, void *handle)
{
    WhirlUI *ui = (WhirlUI *)handle;
    const float val =
        robtk_cbtn_get_active(ui->btn_link) ? (ui->last_used_horn_lever ? 1.f : -1.f) : 0.f;
...
#define INIT_DIAL_SF(VAR, W, H) INIT_DIAL_SF_CLR(VAR, W, H, c_trs);
...
#define DIALLABLEL360(V, TXT, ALIGN)                                                               \
    ...
    {DIALDOTS360(0. / 8, 6.5, 15.5) DIALDOTS360(1. / 8, 6.5, 15.5) DIALLABLEL360(2. / 8, "R", 3)
         DIALDOTS360(3. / 8, 6.5, 15.5) DIALDOTS360(4. / 8, 6.5, 15.5)
             DIALDOTS360(5. / 8, 6.5, 15.5) DIALLABLEL360(6. / 8, "L", 1)
                 DIALDOTS360(7. / 8, 6.5, 15.5)} cairo_destroy(cr);
...
#define ARROW_UP ARROW(5, 5, -10, 0)
    ...
    const float hsc = ui->spk_dpy[0]->area.width < 1.5 * ui->spk_dpy[0]->area.height
                          ? ui->spk_dpy[0]->area.width
                          : 1.5 * ui->spk_dpy[0]->area.height;
...
static bool tblbox_expose_event(RobWidget *rw, cairo_t *cr, cairo_rectangle_t *ev)
{
    ...
    if (rw->resized)
    {
        ...
        event.height =
            MIN(rw->area.y + rw->area.height, ev->y + ev->height) - MAX(ev->y, rw->area.y);
        cairo_save(cr);
        cairo_set_operator(cr, CAIRO_OPERATOR_SOURCE);
        cairo_set_source_rgb(cr, c[0], c[1], c[2]);
...
static bool noop_expose_event(RobWidget *rw, cairo_t *cr, cairo_rectangle_t *ev) { return TRUE; }
...
#define LABEL_BACKGROUND(LBL, R, G, B, A)                                                          \
    ...
    ui->s_drumwidth = robtk_dial_new_with_size(0, 2, .05, GED_WIDTH + 12, GED_HEIGHT + 20,
                                               GED_CX + 6, GED_CY + 15, GED_RADIUS);
    ...
    ui->s_hornwidth = robtk_dial_new_with_size(0, 2, .05, GED_WIDTH + 12, GED_HEIGHT + 20,
                                               GED_CX + 6, GED_CY + 15, GED_RADIUS);
    ...
    ui->s_leak = robtk_dial_new_with_size(-63, -3, .02, GED_WIDTH + 12, GED_HEIGHT + 20, GED_CX + 6,
                                          GED_CY + 15, GED_RADIUS);
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        ui->s_ffreq[i] = robtk_dial_new_with_size(0, 1, 1. / 160, GED_WIDTH + 12, GED_HEIGHT + 20,
                                                  GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ui->s_fqual[i] = robtk_dial_new_with_size(0, 1, 1. / 90, GED_WIDTH + 12, GED_HEIGHT + 20,
                                                  GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ui->s_fgain[i] =
            robtk_dial_new_with_size(filter[i][2].min, filter[i][2].max, 0.2, GED_WIDTH + 12,
                                     GED_HEIGHT + 20, GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ...
        rob_table_attach(ui->tbl_flt[i], LB_W(ui->lbl_flt[i][0]), 0, 3, 0, 1, 3, 4, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_flt[i], SL_W(ui->sel_fil[i]), 0, 3, 1, 2, 4, 0, RTK_EXANDF,
                         RTK_SHRINK);

        rob_table_attach(ui->tbl_flt[i], LB_W(ui->lbl_flt[i][2]), 0, 1, 2, 3, 3, 2, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_flt[i], LB_W(ui->lbl_flt[i][3]), 1, 2, 2, 3, 3, 2, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_flt[i], LB_W(ui->lbl_flt[i][4]), 2, 3, 2, 3, 3, 2, RTK_EXANDF,
                         RTK_SHRINK);

        rob_table_attach(ui->tbl_flt[i], DL_W(ui->s_ffreq[i]), 0, 1, 3, 4, 4, 2, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_flt[i], DL_W(ui->s_fqual[i]), 1, 2, 3, 4, 4, 2, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_flt[i], DL_W(ui->s_fgain[i]), 2, 3, 3, 4, 4, 2, RTK_EXANDF,
                         RTK_SHRINK);
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        ui->s_level[i] = robtk_dial_new_with_size(-20, 20, .02, GED_WIDTH + 12, GED_HEIGHT + 20,
                                                  GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ui->s_brakepos[i] =
            robtk_dial_new_with_size(1. / 360, 1, 1. / 360, GED_WIDTH + 12, GED_HEIGHT + 20,
                                     GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ui->s_rpm_slow[i] = robtk_dial_new_with_size(0, 1, 1. / 96, GED_WIDTH + 12, GED_HEIGHT + 20,
                                                     GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ui->s_rpm_fast[i] = robtk_dial_new_with_size(0, 1, 1. / 96, GED_WIDTH + 12, GED_HEIGHT + 20,
                                                     GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ui->s_accel[i] = robtk_dial_new_with_size(0, 1, 1. / 96, GED_WIDTH + 12, GED_HEIGHT + 20,
                                                  GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ui->s_decel[i] = robtk_dial_new_with_size(0, 1, 1. / 96, GED_WIDTH + 12, GED_HEIGHT + 20,
                                                  GED_CX + 6, GED_CY + 15, GED_RADIUS);
        ...
        robtk_dial_set_default(ui->s_accel[i],
                               param_to_dial(&acceleration[i], acceleration[i].dflt));
        robtk_dial_set_default(ui->s_decel[i],
                               param_to_dial(&deceleration[i], deceleration[i].dflt));
        ...
        LABEL_BACKGROUND(lbl_rpm[i], .21, .21, .27,
                         1.0); // match tblbox_expose_event
        ...
        rob_table_attach(ui->tbl_mtr[i], LB_W(ui->lbl_mtr[i][0]), 0, 2, 0, 1, 2, 8, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], LB_W(ui->lbl_rpm[i]), 0, 2, 1, 2, 2, 2, RTK_EXANDF,
                         RTK_EXANDF);
        rob_table_attach(ui->tbl_mtr[i], LB_W(ui->lbl_mtr[i][1]), 0, 1, 2, 3, 2, 0, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], LB_W(ui->lbl_mtr[i][2]), 0, 1, 3, 4, 2, 0, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], LB_W(ui->lbl_mtr[i][3]), 0, 1, 4, 5, 2, 0, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], LB_W(ui->lbl_mtr[i][4]), 0, 1, 5, 6, 2, 0, RTK_EXANDF,
                         RTK_SHRINK);

        rob_table_attach(ui->tbl_mtr[i], DL_W(ui->s_rpm_slow[i]), 1, 2, 2, 3, 4, 0, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], DL_W(ui->s_rpm_fast[i]), 1, 2, 3, 4, 4, 0, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], DL_W(ui->s_accel[i]), 1, 2, 4, 5, 4, 0, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], DL_W(ui->s_decel[i]), 1, 2, 5, 6, 4, 0, RTK_EXANDF,
                         RTK_SHRINK);
        rob_table_attach(ui->tbl_mtr[i], SP_W(ui->sep_tbl[i]), 0, 2, 6, 7, 0, 0, RTK_EXANDF,
                         RTK_SHRINK);
    }
    ...
    rob_table_attach(ui->tbl_adv[0], LB_W(ui->lbl_adv[0]), 0, 2, 0, 1, 2, 6, RTK_EXANDF,
                     RTK_EXANDF);
    rob_table_attach(ui->tbl_adv[0], LB_W(ui->lbl_adv[1]), 0, 1, 1, 2, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[0], LB_W(ui->lbl_adv[2]), 0, 1, 2, 3, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[0], SP_W(ui->sep_adv[0]), 0, 1, 3, 4, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);

    rob_table_attach(ui->tbl_adv[0], DL_W(ui->s_xzmpos[2]), 1, 2, 1, 2, 0, 2, RTK_SHRINK,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[0], DL_W(ui->s_xzmpos[3]), 1, 2, 2, 3, 0, 2, RTK_SHRINK,
                     RTK_SHRINK);

    rob_table_attach(ui->tbl_adv[1], SP_W(ui->sep_adv[1]), 0, 1, 0, 1, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], LB_W(ui->lbl_adv[3]), 0, 1, 1, 2, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], LB_W(ui->lbl_adv[4]), 0, 1, 2, 3, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], LB_W(ui->lbl_adv[5]), 0, 1, 3, 4, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], LB_W(ui->lbl_adv[6]), 0, 1, 4, 5, 2, 2, RTK_EXANDF,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], SP_W(ui->sep_adv[2]), 0, 1, 5, 6, 2, 2, RTK_EXANDF,
                     RTK_EXANDF);

    rob_table_attach(ui->tbl_adv[1], DL_W(ui->s_radius[0]), 1, 2, 1, 2, 0, 2, RTK_SHRINK,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], DL_W(ui->s_radius[1]), 1, 2, 2, 3, 0, 2, RTK_SHRINK,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], DL_W(ui->s_xzmpos[0]), 1, 2, 3, 4, 0, 2, RTK_SHRINK,
                     RTK_SHRINK);
    rob_table_attach(ui->tbl_adv[1], DL_W(ui->s_xzmpos[1]), 1, 2, 4, 5, 0, 2, RTK_SHRINK,
                     RTK_SHRINK);
    ...
    if (ui->touch)
    {
        ...
        robtk_dial_set_touch(ui->s_rpm_slow[0], ui->touch->touch, ui->touch->handle,
                             B3W_HORNRPMSLOW);
        robtk_dial_set_touch(ui->s_rpm_fast[0], ui->touch->touch, ui->touch->handle,
                             B3W_HORNRPMFAST);
        ...
        robtk_dial_set_touch(ui->s_rpm_slow[1], ui->touch->touch, ui->touch->handle,
                             B3W_DRUMRPMSLOW);
        robtk_dial_set_touch(ui->s_rpm_fast[1], ui->touch->touch, ui->touch->handle,
                             B3W_DRUMRPMFAST);
...
static LV2UI_Handle instantiate(void *const ui_toplevel, const LV2UI_Descriptor *descriptor,
                                const char *plugin_uri, const char *bundle_path,
                                LV2UI_Write_Function write_function, LV2UI_Controller controller,
                                RobWidget **widget, const LV2_Feature *const *features)
{
...
static enum LVGLResize plugin_scale_mode(LV2UI_Handle handle) { return LVGL_LAYOUT_TO_FIT; }
...
{
    WhirlUI *ui = (WhirlUI *)handle;
    if (format != 0 || port_index < B3W_REVSELECT)
        return;
