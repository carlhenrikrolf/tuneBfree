...
| `RotaryPage` (ROTOR) | 4 rows, signal flow left→right: far-left REVERB strip (MatrixVerb DAMP/SIZE/FLAVOR — reverb precedes the whirl; FLAVOR = plate↔spring morph; Speed/Vibrato/Filter deliberately frozen), then per-rotor [motor knobs → speed 3-way → voicing filter], CABINET (radii + x/z offsets, live via computeOffsets) \| drum filter, MIC row \| MIX right-aligned. Speed switches grey while bypassed. |
