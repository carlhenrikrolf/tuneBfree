...
static inline void
MYglGenerateMipmapEXT (GLenum a)
{
	XWglGenerateMipmapEXT (a);
}
static inline void
MYglBindBuffer (GLenum a, GLuint b)
{
	XWglBindBuffer (a, b);
}
static inline void
MYglGenBuffers (GLsizei a, GLuint* b)
{
	XWglGenBuffers (a, b);
}
static inline void
MYglBufferData (GLenum a, GLsizeiptr b, const GLvoid* c, GLenum d)
{
...
static int
show_message (PuglView* view, const char* msg)
{
...
static char*
absfilepath (const char* dir, const char* file)
{
	if (!dir || !file)
		return NULL;
...
static int
check_extension (const char* fn, const char* ext)
{
	if (!fn || !ext)
		return -1;
...
static int
dirlist (PuglView* view, const char* dir)
{
	B3ui*          ui = (B3ui*)puglGetHandle (view);
	DIR*           D;
	struct dirent* dd;
	...
	while ((dd = readdir (D))) {
		...
		if (S_ISREG (fs.st_mode)) {
			...
			if ((strcmp (&rfn[fnl - 4], ".pgm") && strcmp (&rfn[fnl - 4], ".cfg")) || (ui->dir_hidedotfiles && dd->d_name[0] == '.')) {
				free (rfn);
				continue;
			}
...
static void
vmap_midi_to_val (PuglView* view, int elem, int mval)
{
	B3ui* ui            = (B3ui*)puglGetHandle (view);
...
vmap_val_to_midi (PuglView* view, int elem)
{
	B3ui*     ui = (B3ui*)puglGetHandle (view);
...
static void
b3_forge_message (B3ui* ui, const char* key, int32_t val)
{
...
static void
notifyPlugin (PuglView* view, int elem)
{
	B3ui*   ui = (B3ui*)puglGetHandle (view);
	int32_t val;
...
static void
forge_message_kv (B3ui* ui, LV2_URID uri, int key, const char* val)
{
	uint8_t obj_buf[256];
...
static void
forge_message_str (B3ui* ui, LV2_URID uri, const char* key)
{
...
static void
forge_message_int (B3ui* ui, LV2_URID uri, const int val)
{
	uint8_t obj_buf[64];
...
static void
forge_note (B3ui* ui, const int manual, const int note, const bool onoff)
{
	uint8_t obj_buf[16];
...
static void
processCCevent (B3ui* ui, const char* k, int v)
{
	int i;
	/* special cases */
...
static void
processMotion (PuglView* view, int elem, float dx, float dy)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
static void
project_mouse (PuglView* view, int mx, int my, float zo, float* x, float* y)
{
	B3ui*        ui = (B3ui*)puglGetHandle (view);
...
static void
drawMesh (PuglView* view, unsigned int index, int apply_transformations)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
int
save_cfgpgm (PuglView* view, const char* fn, int mode, int override)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
void
handle_msg_reply (PuglView* view)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
static int
txtentry_start (PuglView* view, const char* title, const char* defaulttext)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
static void
txtentry_end (PuglView* view, const char* txt)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
	if (!txt || strlen (txt) == 0) {
...
static void
txtentry_handle (PuglView* view, uint32_t key)
{
	B3ui* ui  = (B3ui*)puglGetHandle (view);
...
static void
piano_manual (PuglView* view, float y0, float z0, int active_key, unsigned int* active_keys)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
static void
piano_pedals (PuglView* view, int active_key, unsigned int active_keys)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
static float
coeff_to_db (const float c, float min120)
{
	return c >= 1e-6 ? 20.0 * log10f (c) : min120;
}

static float
db_to_coeff (const float c)
{
	if (c < -120.f)
		return 0.0;
...
static const ConfigDoc*
searchDoc (const ConfigDoc* d, const char* key)
{
	while (d && d->name) {
...
static void
cfg_initialize_param (B3ui* ui, const char* cfgkey, int p)
{
...
static const char*
lut_lookup_value (b3scalepoint const* const lut, const float val)
{
	int ii = 0;
	if (!lut)
		return NULL;
...
static void
cfg_parse_config (B3ui* ui, const char* key, const char* value)
{
	int relevant = 0;
	int i;
...
static void
cfg_special_button (PuglView* view, int ccc)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
	assert (ui->cfgtab == 0);
...
static void
cfg_start_drag (B3ui* ui, int cfg)
{
	assert (cfg > 0 && cfg <= 24);
...
static float
cfg_update_parameter (B3ui* ui, int ccc, float val, int delta)
{
...
static void
cfg_update_value (PuglView* view, int ccc, int dir)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
...
static int
cfg_mousepos (const float fx, const float fy, int* tri)
{
	int xh = -1;
	int yh = -1;
...
helpscreentext (PuglView* view)
{
	...
	yto = -.61;
	xm0 = -.79;
	render_gl_text (view, "Interaction with the synth is done", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "with the mouse: either via click+drag", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "or for fine-grained control using the", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "scroll-wheel on an element.", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	yto += .05;
	render_gl_text (view, "All actions can be triggered as well", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "by using MIDI-CC messages.", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	yto += .05;
	render_gl_text (view, "The communication is bidirectional:", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "GUI updates will be sent as feedback", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "to the MIDI output, incoming MIDI-CC", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "message update the GUI.", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	yto += .05;
	render_gl_text (view, "In Key-control mode the PC-keyboard", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "can be used to control the elements,", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	render_gl_text (view, "overriding the default key-bindings.", SCOORD (xm0, yto), mat_w, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	...
	render_gl_text (view, "Recall MIDI Program:", SCOORD (xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	...
	render_gl_text (view, "(Re) Assign MIDI-CC:", SCOORD (xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	...
	render_gl_text (view, "Export .cfg file:", SCOORD (xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
	...
	yto += .10;
	render_gl_text (view, "?", SCOORD (xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
	render_gl_text (view, "Toggle Help Text:", SCOORD (xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM, FS_MEDIUM, 0);
	yto += .10;
...
keybindinghelp (PuglView* view)
{
	...
	render_small_text (view, "first select which drawbars are affected:", SCOORD ((xm0 + xm1) / 2, yto), mat_w, TA_CENTER_BOTTOM);
	yto += .1;
	...
	yto += .15;
	render_small_text (view, "Drawbars are mapped left to right:", SCOORD ((xm0 + xm1) / 2, yto), mat_w, TA_CENTER_BOTTOM);
	yto += .15;
	...
	render_small_text (view, "Switches are mapped left-to right as seen on the organ", SCOORD ((xm0 + xm1) / 2, yto), mat_w, TA_CENTER_BOTTOM);
	yto += .075;
	render_small_text (view, "to the lower row of the keyboard Z-M:", SCOORD ((xm0 + xm1) / 2, yto), mat_w, TA_CENTER_BOTTOM);
	yto += .175;
...
static void
reset_state_ccbind (PuglView* view)
{
	B3ui* ui = (B3ui*)puglGetHandle (view);
	if (ui->uiccbind >= 0) {
		ui->uiccbind = -1;
	...
	for (i = 0; i < TOTAL_OBJ; ++i) {
		...
		switch (ui->ctrls[i].type) {
			case OBJ_DIAL:
				...
				if (ui->ctrls[i].max == 0) {
					glRotatef (
					    240.0 - (360.0 * rint (ui->ctrls[i].cur - ui->ctrls[i].min) / (1.0 + ui->ctrls[i].max - ui->ctrls[i].min)), 0, 0, 1);
				} else {
					glRotatef (
					    150.0 - (300.0 * rint (ui->ctrls[i].cur - ui->ctrls[i].min) / (ui->ctrls[i].max - ui->ctrls[i].min)), 0, 0, 1);
				}
				break;
			case OBJ_SWITCH:
			...
			case OBJ_LEVER:
				...
				glMaterialfv (GL_FRONT, GL_EMISSION, no_mat);
				glRotatef (
				    (-40.0 + 80.0 * rint (ui->ctrls[i].cur - ui->ctrls[i].min) / (ui->ctrls[i].max - ui->ctrls[i].min)), 0, 1, 0);
				break;
			default:
				break;
		}
	...
	switch (ui->displaymode) {
		...
		case 6: //IS_FILEBROWSER() == displaymode 4,5,6
			...
			if (ui->dir_sel >= 0 && ui->dir_sel < ui->dirlistlen) {
			    ...
			    ui->dirlistlen > 120 && MOUSEIN (SCROLLBAR, fx, fy)) {
				// handle scrollbar
				int   pages = (ui->dirlistlen / 20);
				float ss    = 1.6 / (float)pages;
				float sw    = 5.0 * ss;
			    ...
			    (ui->displaymode == 5 || ui->displaymode == 6) && MOUSEIN (BTNLOC_SAVE, fx, fy)) {
				txtentry_start (view, "Enter File Name:", "");
				return;
...
static void
x_run (struct lv2_external_ui* handle)
{
...
static void
x_show (struct lv2_external_ui* handle)
{
	B3ui* ui = (B3ui*)handle->self;
...
static void
x_hide (struct lv2_external_ui* handle)
{
	B3ui* ui = (B3ui*)handle->self;
	puglHideWindow (ui->view);
}
...
static int
sb3_gui_setup (B3ui* ui, const LV2_Feature* const* features)
{
            ...
            const void*  buffer)
{
	...
	if (obj->body.otype == ui->uris.sb3_activekeys) {
		LV2_Atom* a0 = NULL;
		if (1 == lv2_atom_object_get (obj, ui->uris.sb3_keyarrary, &a0, NULL) && a0->type == ui->uris.atom_Vector) {
