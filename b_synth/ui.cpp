...
#ifdef XTERNAL_UI
#undef OLD_SUIL
#include "../libs/robtk/gl/xternalui.h"
#endif
...
static int glext_func()
{
    ...
    XWglBufferData = (__stdcall void (*)(GLenum, GLsizeiptr, const GLvoid *,
                                         GLenum))wglGetProcAddress("glBufferData");
...
static inline void MYglGenerateMipmapEXT(GLenum a) { XWglGenerateMipmapEXT(a); }
static inline void MYglBindBuffer(GLenum a, GLuint b) { XWglBindBuffer(a, b); }
static inline void MYglGenBuffers(GLsizei a, GLuint *b) { XWglGenBuffers(a, b); }
...
static const char *obj_control[] = {
    "upper.drawbar16", // 0
    "upper.drawbar513",    "upper.drawbar8",   "upper.drawbar4",      "upper.drawbar223",
    "upper.drawbar2",      "upper.drawbar135", "upper.drawbar113",
    "upper.drawbar1", // 8

    "lower.drawbar16",     "lower.drawbar513", "lower.drawbar8",      "lower.drawbar4",
    "lower.drawbar223",    "lower.drawbar2",   "lower.drawbar135",    "lower.drawbar113",
    "lower.drawbar1", // 17

    "pedal.drawbar16",     "pedal.drawbar8",

    "percussion.enable", // 20
    "percussion.volume",   "percussion.decay", "percussion.harmonic",
...
typedef struct
{
    ...
    float dflt; // default value, parsed float from textual ConfigDoc, if
                // applicable
    ConfigDoc const *d;
...
static int dirlist(PuglView *view, const char *dir)
{
    ...
    while ((dd = readdir(D)))
    {
        ...
        if (delen == 1 && dd->d_name[0] == '.')
            continue; // '.'
        else if (delen == 2 && dd->d_name[0] == '.' && dd->d_name[1] == '.')
            ; // '..' -> OK
        else if (ui->dir_hidedotfiles && dd->d_name[0] == '.')
            continue;
...
static void vmap_midi_to_val(PuglView *view, int elem, int mval)
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
    ui->ctrls[elem].cur =
        ui->ctrls[elem].min + rint((ui->ctrls[elem].max - ui->ctrls[elem].min) * mval / 127.0);
}

static unsigned char vmap_val_to_midi(PuglView *view, int elem)
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
    const int v = rint(rint(ui->ctrls[elem].cur - ui->ctrls[elem].min) * 127.0 /
                       (ui->ctrls[elem].max - ui->ctrls[elem].min));
    return (v & 0x7f);
}
...
static void forge_note(B3ui *ui, const int manual, const int note, const bool onoff)
{
    ...
    ui->write(ui->controller, 0, lv2_atom_total_size(&midiatom), ui->uris.atom_eventTransfer,
              obj_buf);
}
...
static void processCCevent(B3ui *ui, const char *k, int v)
{
    ...
    else if (!strcmp("rotary.speed-select", k))
    {
...
static void processMotion(PuglView *view, int elem, float dx, float dy)
{
    ...
    case OBJ_DIAL:
        ...
        else
        {
            if (ui->ctrls[elem].cur > ui->ctrls[elem].max)
                ui->ctrls[elem].cur = ui->ctrls[elem].max;
            if (ui->ctrls[elem].cur < ui->ctrls[elem].min)
                ui->ctrls[elem].cur = ui->ctrls[elem].min;
        }
        break;
    case OBJ_LEVER:
...
static void project_mouse(PuglView *view, int mx, int my, float zo, float *x, float *y)
{
    ...
    const double fz =
        -ui->matrix[14] - (fx * ui->matrix[2] + fy * ui->matrix[6] - SCALE * zo) / ui->matrix[10];
...
static void initTextures(PuglView *view)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, wood_image.width, wood_image.height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, wood_image.pixel_data);
    if (atihack)
    {
        glEnable(GL_TEXTURE_2D);
        glGenerateMipmapEXT(GL_TEXTURE_2D);
...
static void render_gl_text(PuglView *view, const char *text, float x, float y, float z,
                           const GLfloat color[4], B3TextAlign align, B3FontSize fs, int blend)
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
  ...
  printf("%.2f %.2f %.2f  %.2f %.2f %.2f\n",
    ...
    {
    case TA_CENTER_TOP:
        glTranslatef((bb[3] - bb[0]) / -2.0, (bb[4] - bb[1]) / -1.0, 0);
        break;
    case TA_CENTER_MIDDLE:
        glTranslatef((bb[3] - bb[0]) / -2.0, (bb[4] - bb[1]) / -2.0, 0);
        break;
    case TA_RIGHT_TOP:
        glTranslatef((bb[3] - bb[0]) / -1.0, (bb[4] - bb[1]) / -1.0, 0);
        break;
    case TA_LEFT_BOTTOM:
        break;
    case TA_RIGHT_BOTTOM:
        glTranslatef((bb[3] - bb[0]) / -1.0, 0, 0);
        break;
    case TA_CENTER_BOTTOM:
        glTranslatef((bb[3] - bb[0]) / -2.0, 0, 0);
        break;
    case TA_LEFT_MIDDLE:
        glTranslatef(0, (bb[4] - bb[1]) / -2.0, 0);
        break;
    case TA_LEFT_TOP:
        glTranslatef(0, (bb[4] - bb[1]) / -1.0, 0);
        break;
    }
...
static void render_title(PuglView *view, const char *text, float x, float y, float z,
                         const GLfloat color[4], B3TextAlign align)
{
    render_gl_text(view, text, x, y, z / SCALE, color, align, FS_LARGE, 0);
}

static void render_small_text(PuglView *view, const char *text, float x, float y, float z,
                              const GLfloat color[4], B3TextAlign align)
{
    render_gl_text(view, text, x, y, z, color, align, FS_SMALL, 1);
}

static void render_text(PuglView *view, const char *text, float x, float y, float z,
                        B3TextAlign align)
{
...
static void unity_box(PuglView *view, const float x0, const float x1, const float y0,
                      const float y1, const GLfloat color[4])
{
    assert(x0 < x1);
    assert(y0 < y1);
...
static void unity_button_color(PuglView *view, const float x0, const float x1, const float y0,
                               const float y1, GLfloat btncol[3])
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
    const float invaspect = 320. / 960.;
    float x0A, x0B;

    /* button texture: x/3 (left-circle + x/3 (box) + x/3 (right-circle) ;;
     * texture aspect: 4/3
     * ->  x/3 == (y * 4/3) / 3
     */
    ...
    glVertex3f(x0A, y1 * invaspect, 0);
    glTexCoord2f(0.33, 0.0);
    glVertex3f(x0A, y0 * invaspect, 0);
    ...
    glVertex3f(x0B, y1 * invaspect, 0);
    glTexCoord2f(0.66, 0.0);
    glVertex3f(x0B, y0 * invaspect, 0);
...
static void unity_button(PuglView *view, const float x0, const float x1, const float y0,
                         const float y1, int hover)
{
...
static void gui_button(PuglView *view, const float x0, const float x1, const float y0,
                       const float y1, int hovermask, const char *label)
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
    const GLfloat mat_white[] = {1.0, 1.0, 1.0, 1.0};
    const float invaspect = 320. / 960.;
    unity_button(view, x0, x1, y0, y1, ui->mouseover & hovermask);
    render_title(view, label, (x1 + x0) / 2.0 / SCALE, invaspect * (y1 + y0) / 2.0 / SCALE, 0.5,
                 mat_white, TA_CENTER_MIDDLE);
}

static void menu_button(PuglView *view, const float x0, const float x1, const float y0,
                        const float y1, int texid, int hovermask, const char *label)
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
    const float invaspect = 320. / 960.;
    ...
    if (texid > 0)
    {
        ...
        glTexCoord2f(0.0, 0.0);
        glVertex3f(x0, invaspect * y0, 0);
        glTexCoord2f(0.0, 1.0);
        glVertex3f(x0, invaspect * y1, 0);
        glTexCoord2f(1.0, 1.0);
        glVertex3f(x1, invaspect * y1, 0);
        glTexCoord2f(1.0, 0.0);
        glVertex3f(x1, invaspect * y0, 0);
        glEnd();
        glBlendFunc(GL_SRC_ALPHA, GL_SRC_ALPHA_SATURATE);
        glDisable(GL_TEXTURE_2D);
        glDisable(GL_BLEND);
        if (ui->mouseover & hovermask)
        {
            ...
            render_title(view, label, (x1 + x0) / 2.0 / SCALE, invaspect * (y1 + y0) / 2.0 / SCALE,
    ...
    else
    {
        ...
        unity_button(view, x0, x1, y0, y1, ui->mouseover & hovermask);
        render_title(view, label, (x1 + x0) / 2.0 / SCALE, invaspect * (y1 + y0) / 2.0 / SCALE, 0.5,
                     mat_w, TA_CENTER_MIDDLE);
    }
}

static void unity_tri(PuglView *view, const float x0, const float y0, const float y1,
...
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
    if (!ui->pendingdata || !ui->pendingmode)
        return;
...
static void piano_manual(PuglView *view, float y0, float z0, int active_key,
                         unsigned int *active_keys)
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
...
static const ConfigDoc *searchDocs(const char *key)
{
    ...
    if (!rv)
        rv = searchDoc(midiDoc(), key);
    if (!rv)
        rv = searchDoc(pgmDoc(), key);
    if (!rv)
        rv = searchDoc(oscDoc(), key);
    if (!rv)
        rv = searchDoc(scannerDoc(), key);
    if (!rv)
        rv = searchDoc(ampDoc(), key);
    if (!rv)
        rv = searchDoc(whirlDoc(), key);
    if (!rv)
        rv = searchDoc(reverbDoc(), key);
    return rv;
}
...
static void cfg_initialize_param(B3ui *ui, const char *cfgkey, int p)
{
    ...
    case CFG_INT:
    {
        assert(ui->cfgvar[p].format == CF_DECIBEL || ui->cfgvar[p].format == CF_NUMBER ||
               ui->cfgvar[p].format == CF_PERCENT || ui->cfgvar[p].format == CF_DEGREE ||
               ui->cfgvar[p].format == CF_INTEGER);
...
static const b3scalepoint x_temperament[] = {{0, "gear60"}, {1, "gear50"}, {2, "equal"}, {0, NULL}};

static const b3scalepoint x_contactmodel[] = {
    {0, "click"}, {1, "shelf"}, {2, "cosine"}, {3, "linear"}, {0, NULL}};

static const b3scalepoint x_filtertype[] = {{0, "Low Pass"},
                                            {1, "High Pass"},
...
static const b3scalepoint x_bypass[] = {
    {0, "off (effect is active)"}, {1, "on (effect is disabled)"}, {0, NULL}};
...
static void cfg_initialize(B3ui *ui)
{
    ...
    CFGP("osc.temperament", "Temperament", CF_LISTLUT,
         x_temperament); // 'gear60'
  ...
  CFGP("midi.pedals.transpose.split", "Splitpoint for Pedals",       CF_INTEGER, NULL);
    ...
    // CFGP("osc.transformer-crosstalk", "trans. X-Talk",        CF_DECIBEL,
    // NULL);
...
static void cfg_start_drag(B3ui *ui, int cfg)
{
    ...
    {
    case CF_DECIBEL:
        range = fabsf(coeff_to_db(ui->cfgvar[ccc].d->max, -120) -
                      coeff_to_db(ui->cfgvar[ccc].d->min, -120));
        break;
    default:
    ...
    ui->dragmult /= 350; // px
                         ...
                         // range, ui->dragmult, 1.0 / ui->dragmult);
}
...
static void cfg_tx_update(B3ui *ui, int ccc)
{
    ...
    {
    case CF_LISTLUT:
        snprintf(cfgstr, sizeof(cfgstr), "%s=%s", ui->cfgvar[ccc].d->name,
                 ui->cfgvar[ccc].lut[(int)rint(ui->cfgvar[ccc].cur)].label);
        break;
    case CF_INTEGER:
        snprintf(cfgstr, sizeof(cfgstr), "%s=%.0f", ui->cfgvar[ccc].d->name, ui->cfgvar[ccc].cur);
        break;
    case CF_PERCENT:
    case CF_DEGREE:
        snprintf(cfgstr, sizeof(cfgstr), "%s=%.4f", ui->cfgvar[ccc].d->name, ui->cfgvar[ccc].cur);
        break;
    default:
        snprintf(cfgstr, sizeof(cfgstr), "%s=%.10f", ui->cfgvar[ccc].d->name, ui->cfgvar[ccc].cur);
        break;
    }
    LOCALEGUARD_END;
...
static void render_cfg_button(PuglView *view, int ccc, float x0, float x1, float y0, float y1,
                              int hover_btn, int hover_tri, bool dragging)
{
    ...
    case CF_INTEGER:
        ...
        {
            snprintf(txt, sizeof(txt), "%s: %s", ui->cfgvar[ccc].title, lbl);
        }
        else
        {
            snprintf(txt, sizeof(txt), "%s: %.0f %s", ui->cfgvar[ccc].title, val,
                     ui->cfgvar[ccc].d->unit);
        }
        break;
    case CF_PERCENT:
        if ((lbl = lut_lookup_value(ui->cfgvar[ccc].lut, val)))
        ...
        {
            snprintf(txt, sizeof(txt), "%s: %.1f%s", ui->cfgvar[ccc].title, 100.f * val,
                     ui->cfgvar[ccc].d->unit);
        }
        break;
    case CF_DEGREE:
        ...
        {
            snprintf(txt, sizeof(txt), "%s: %.1f%s", ui->cfgvar[ccc].title, 360.f * val,
                     ui->cfgvar[ccc].d->unit);
        }
        break;
    case CF_DECIBEL:
        ...
        {
            snprintf(txt, sizeof(txt), "%s: %+.0f%s", ui->cfgvar[ccc].title,
                     coeff_to_db(val, -INFINITY), ui->cfgvar[ccc].d->unit);
        }
        break;
    default:
        if ((lbl = lut_lookup_value(ui->cfgvar[ccc].lut, val)))
        ...
        {
            snprintf(txt, sizeof(txt), "%s: %.2f%s", ui->cfgvar[ccc].title, val,
                     ui->cfgvar[ccc].d->unit);
        }
        break;
    }

    render_small_text(view, txt, (x1 + x0) / 2.0 / SCALE, invaspect * (y1 + y0) / 2.0 / SCALE, 0.5,
                      hover_btn ? mat_b : mat_w, TA_CENTER_MIDDLE);
}
...
static int cfg_tabbar(const float fx)
{
    if (fx > -.975 && fx < -.625)
        return 0;
    else if (fx > -.575 && fx < -.225)
        return 1;
    else if (fx > -.175 && fx < .175)
        return 2;
    else if (fx > .226 && fx < .575)
        return 3;
    else if (fx > .625 && fx < .975)
        return 4;
    return -1;
}
...
static void advanced_config_screen(PuglView *view)
{
    B3ui *ui = (B3ui *)puglGetHandle(view);
    const GLfloat mat_w[] = {1.0, 1.0, 1.0, 1.0};
    const GLfloat mat_b[] = {0.0, 0.0, 0.0, 1.0};
...
#define TABTXTCOLOR(x) ((ui->cfgtab == x) ? mat_w : mat_hv)
    ...
    if (mouseover > 0 && mouseover <= 24)
    {
        ...
        {
            render_small_text(view, "Description (see the manual for complete info):", -23.75, 7.5,
                              0.5, mat_g, TA_LEFT_BOTTOM);
            render_small_text(view, ui->cfgvar[ccc].d->desc, -23.75, 8.22, 0.5, mat_w,
                              TA_LEFT_BOTTOM);
        }
    }
    ...
    if (ui->cfgtab == 0)
    {
        ...
        render_small_text(view,
                          "setBfree is a 'Tonewheel Organ Construction Kit' with over 1000 "
                          "configurable parameters.",
                          0, yto, 0.5, mat_w, TA_CENTER_BOTTOM);
        yto += .75;
        render_small_text(view,
                          "This dialog only exposes some more common 'advanced' settings. Use a "
                          "config file for complete control.",
                          0, yto, 0.5, mat_w, TA_CENTER_BOTTOM);
        yto += .75;
        render_small_text(view, "NOTE: changing any of these parameters re-initializes the synth.",
                          0, yto, 0.5, mat_w, TA_CENTER_BOTTOM);
        yto += .75;
        yto += .5;
        render_small_text(view,
                          "Shift + Click on an element to restore its setting to "
                          "the default value.",
                          0, yto, 0.5, mat_w, TA_CENTER_BOTTOM);
        yto += .75;
        render_small_text(view,
                          "Click and drag on the button for large changes, click "
                          "on the arrows for "
                          "stepwise adjustment.",
                          0, yto, 0.5, mat_w, TA_CENTER_BOTTOM);
        yto += .75;
        render_small_text(view,
                          "Hold Ctrl to alter the granularity (fine graind drag; "
                          "or large click-steps).",
                          0, yto, 0.5, mat_w, TA_CENTER_BOTTOM);
        yto += .75;
    }
    ...
    for (xh = 0; xh < 4; ++xh)
    {
        for (yh = 0; yh < 6; ++yh)
        {
            ...
            render_cfg_button(view, ccc, ccx, ccx + .4, ccy, ccy + .15, mouseover == ccm + 1,
                              ui->cfgtriover, ui->cfgdrag == ccm + 1);
        }
    }
    ...
    if (ui->cfgtab == 0)
    {
        ...
        render_small_text(view, "Factory Reset", (x0 + .2) / SCALE, invaspect * (y0 + .075) / SCALE,
                          0.5, mouseover == 22 ? mat_b : mat_w, TA_CENTER_MIDDLE);

        x0 = -.95 + .5 * 2;
        y0 = -.70 + .25 * 5;
        unity_button(view, x0, x0 + .4, y0, y0 + .15, mouseover == 23);
        render_small_text(view, "Reset, keep MIDI & Program", (x0 + .2) / SCALE,
                          invaspect * (y0 + .075) / SCALE, 0.5, mouseover == 23 ? mat_b : mat_w,
                          TA_CENTER_MIDDLE);
    }
}
...
static void helpscreentext(PuglView *view)
{
    ...
    yto = -.61;
    xm0 = -.79;
    render_gl_text(view, "Interaction with the synth is done", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "with the mouse: either via click+drag", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "or for fine-grained control using the", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "scroll-wheel on an element.", SCOORD(xm0, yto), mat_w, TA_LEFT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    yto += .05;
    render_gl_text(view, "All actions can be triggered as well", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "by using MIDI-CC messages.", SCOORD(xm0, yto), mat_w, TA_LEFT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    yto += .05;
    render_gl_text(view, "The communication is bidirectional:", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "GUI updates will be sent as feedback", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "to the MIDI output, incoming MIDI-CC", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "message update the GUI.", SCOORD(xm0, yto), mat_w, TA_LEFT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    yto += .05;
    render_gl_text(view, "In Key-control mode the PC-keyboard", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "can be used to control the elements,", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "overriding the default key-bindings.", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    yto += .10;

    yto += .02;
    render_gl_text(view, "Version " VERSION " - setbfree.org", SCOORD(xm0, yto), mat_w,
                   TA_LEFT_BOTTOM, FS_SMALL, 0);

    //
    render_gl_text(view, "Keyboard Shortcuts", SCOORD(.22, -.82), mat_w, TA_CENTER_BOTTOM,
                   FS_MEDIUM, 0);

    yto = -.61;
    xm0 = .27;
    render_gl_text(view, "P", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Recall MIDI Program:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "<Shift>P", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Store MIDI Program:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    yto += .06;
    render_gl_text(view, "M", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Display CC-map:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "<Ctrl>Btn2", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "(Re) Assign MIDI-CC:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "(hold <Shift> to invert mapped value)", SCOORD(xm0 - .06, yto), mat_w,
                   TA_CENTER_BOTTOM, FS_SMALL, 0);
    yto += .10;
    yto += .07;

    render_gl_text(view, "<Shift><Space>", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Config Animation:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "~", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Config Editor:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    yto += .07;

    render_gl_text(view, "<Shift>L", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Load .pgm/.cfg:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "<Shift>C", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Export .cfg file:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "<Shift>V", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Export .pgm file:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    yto += .07;

    render_gl_text(view, "<Tab>", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Toggle Key Control:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "?", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM, FS_MEDIUM, 0);
    render_gl_text(view, "Toggle Help Text:", SCOORD(xm0 - .01, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .10;
    render_gl_text(view, "(In \"Key-Control\" mode '?' show the keyboard map)",
                   SCOORD(xm0 - .06, yto), mat_w, TA_CENTER_BOTTOM, FS_SMALL, 0);
    ...
    xm1 = .9;
    xm2 = .95;
    render_gl_text(view, "J", SCOORD(xm1, yto), mat_r, TA_CENTER_BOTTOM, FS_MEDIUM, 0);
    yto += .15;
    ...
    render_gl_text(view, "Turn (Z Axis):", SCOORD(xm0 - .03, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);

    yto += .25;

    render_gl_text(view, "W", SCOORD(xm1, yto), mat_r, TA_CENTER_BOTTOM, FS_MEDIUM, 0);
    yto += .15;
    ...
    render_gl_text(view, "Tilt (X,Y Axis):", SCOORD(xm0 - .03, yto), mat_w, TA_RIGHT_BOTTOM,
                   FS_MEDIUM, 0);
    yto += .15;
    ...
    render_gl_text(view, "3D Presets:", SCOORD(xm0 - .03, yto), mat_w, TA_RIGHT_BOTTOM, FS_MEDIUM,
                   0);
}
...
static void keybindinghelp(PuglView *view)
{
    ...
    render_title(view, "-- [Computer] Keyboard Control --", SCOORD(0, -.95), mat_w,
                 TA_CENTER_MIDDLE);
    render_small_text(view,
                      "Toggle keyboard-grab with <Tab>. When enabled, organ-controls can be "
                      "modified with by keypresses. Currently a US-104 keyboard layout is "
                      "assumed.",
                      SCOORD(0, -.80), mat_w, TA_CENTER_MIDDLE);
    render_small_text(view, "[click or press '?' to close this help]", .2 / SCALE,
                      .15 * invaspect / SCALE, 0.5, mat_w, TA_CENTER_BOTTOM);
    ...
    render_small_text(view, "(recall progam 1-9)", SCOORD((xm0 + xm1) / 2, yto + .15), mat_w,
                      TA_CENTER_BOTTOM);
    ...
    render_small_text(view,
                      "first select which drawbars are affected:", SCOORD((xm0 + xm1) / 2, yto),
                      mat_w, TA_CENTER_BOTTOM);
    yto += .1;
    render_title(view, "[  ]  \\ ", SCOORD(xm1, yto), mat_r, TA_RIGHT_BOTTOM);
    render_small_text(view, "(select: pedal, lower, upper)", SCOORD(xm0, yto), mat_w,
                      TA_LEFT_BOTTOM);
    yto += .15;
    render_small_text(view, "Drawbars are mapped left to right:", SCOORD((xm0 + xm1) / 2, yto),
                      mat_w, TA_CENTER_BOTTOM);
    yto += .15;
    render_title(view, "Q W E R T Y U I O", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM);
    render_small_text(view, "(push drawbar in)", SCOORD(xm1, yto), mat_w, TA_RIGHT_BOTTOM);
    yto += .2;
    ...
    render_small_text(view, "(Volume)", SCOORD(xm1, yto), mat_w, TA_RIGHT_BOTTOM);
    yto += .2;
    render_title(view, "; '", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM);
    render_small_text(view, "(Reverb)", SCOORD(xm1, yto), mat_w, TA_RIGHT_BOTTOM);
    yto += .2;
    render_title(view, "< >", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM);
    render_small_text(view, "(Overdrive)", SCOORD(xm1, yto), mat_w, TA_RIGHT_BOTTOM);
    yto += .2;
    render_title(view, ", .", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM);
    ...
    render_small_text(view, "Switches are mapped left-to right as seen on the organ",
                      SCOORD((xm0 + xm1) / 2, yto), mat_w, TA_CENTER_BOTTOM);
    yto += .075;
    render_small_text(view, "to the lower row of the keyboard Z-M:", SCOORD((xm0 + xm1) / 2, yto),
                      mat_w, TA_CENTER_BOTTOM);
    yto += .175;
    render_title(view, "Z X C V B N M", SCOORD(xm0, yto), mat_r, TA_LEFT_BOTTOM);
    render_small_text(view, "(overdrive, 2 x vibratos, 4 x percussion)", SCOORD(xm1, yto), mat_w,
                      TA_RIGHT_BOTTOM);
    ...
    render_title(view, "<Shift>+<Space>", SCOORD(xm0 + (xm1 - xm0) * 3 / 4, yto), mat_r,
                 TA_CENTER_BOTTOM);
    yto += .10;
    render_small_text(view, "(linked horn + baffle; iterate 3 states)",
                      SCOORD(xm0 + (xm1 - xm0) / 4, yto), mat_w,
                      TA_CENTER_BOTTOM); // yto+=.2;
    render_small_text(view, "(iterate over all 9 states)", SCOORD(xm0 + (xm1 - xm0) * 3 / 4, yto),
                      mat_w, TA_CENTER_BOTTOM);
}
...
static void onDisplay(PuglView *view)
{
    ...
    if (!ui->initialized)
    {
        ...
        for (i = 0; i < ic; ++i)
        {
            if (strstr(_dyld_get_image_name(i), "/b_synthUI.dylib") ||
                strstr(_dyld_get_image_name(i), "/setBfreeUI"))
            {
    ...
    if (ui->popupmsg)
    {
        ...
        if (ui->pendingmode)
        {
            render_text(view, "Press <enter> to confirm, <ESC> to abort, or press button.", 0, 7.0,
                        0, TA_CENTER_MIDDLE);
    ...
    else if (ui->displaymode == 2 || ui->displaymode == 3)
    {
        ...
        render_title(view, (ui->displaymode == 2) ? "recall" : "store", 16.5, -.75, 0.1,
                     mat_drawbar_white, TA_LEFT_BOTTOM);
    ...
    else if (IS_FILEBROWSER(ui))
    {
        ...
        {
        case 4:
            render_title(view, "open .pgm or .cfg", .2 / SCALE, invaspect * .78 / SCALE, 0.1,
                         mat_drawbar_white, TA_RIGHT_TOP);
            render_text(view, "Note: loading a .cfg will re-initialize the organ.", -20.0, 7.75,
                        0.0, TA_LEFT_BOTTOM);
#ifdef JACK_DESCRIPT
            ...
            break;
        case 5:
            render_title(view, ".cfg", -.92 / SCALE, invaspect * .85 / SCALE, 0.1,
                         mat_drawbar_white, TA_CENTER_MIDDLE);
            render_title(view, "save", -.92 / SCALE, invaspect * .75 / SCALE, 0.1,
                         mat_drawbar_white, TA_CENTER_MIDDLE);
...
#endif
            render_text(view, "select a file or press OK or <enter> to create new.", -20.0, 7.75,
                        0.0, TA_LEFT_BOTTOM);
            break;
        case 6:
            render_title(view, ".pgm", -.92 / SCALE, invaspect * .85 / SCALE, 0.1,
                         mat_drawbar_white, TA_CENTER_MIDDLE);
            render_title(view, "save", -.92 / SCALE, invaspect * .75 / SCALE, 0.1,
                         mat_drawbar_white, TA_CENTER_MIDDLE);
            gui_button(view, BTNLOC_SAVE, HOVER_SAVE, "Save");
    ...
    if (1)
    {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0.0, 0.0);
        glVertex3f(0.92, -.25, 0);
        glTexCoord2f(0.0, 1.0);
        glVertex3f(0.92, -.16, 0);
        glTexCoord2f(1.0, 1.0);
        glVertex3f(1.1, -.16, 0);
        glTexCoord2f(1.0, 0.0);
        glVertex3f(1.1, -.25, 0);
    ...
    for (i = 0; i < TOTAL_OBJ; ++i)
    {
        ...
        if (ui->ctrls[i].type == OBJ_DRAWBAR)
        { /* drawbar */
            y += 6.0;
        ...
        case OBJ_DIAL:
            ...
            if (ui->ctrls[i].max == 0)
            {
                glRotatef(240.0 - (360.0 * rint(ui->ctrls[i].cur - ui->ctrls[i].min) /
                                   (1.0 + ui->ctrls[i].max - ui->ctrls[i].min)),
                          0, 0, 1);
            }
            else
            {
                glRotatef(150.0 - (300.0 * rint(ui->ctrls[i].cur - ui->ctrls[i].min) /
                                   (ui->ctrls[i].max - ui->ctrls[i].min)),
                          0, 0, 1);
            }
            break;
        case OBJ_SWITCH:
            ...
            glRotatef((vmap_val_to_midi(view, i) < 64 ? -12 : 12.0), 1, 0,
                      0); // XXX
            break;
        case OBJ_DRAWBAR:
            glMaterialfv(GL_FRONT, GL_EMISSION, no_mat);
        ...
        case OBJ_LEVER:
            ...
            glMaterialfv(GL_FRONT, GL_EMISSION, no_mat);
            glRotatef((-40.0 + 80.0 * rint(ui->ctrls[i].cur - ui->ctrls[i].min) /
                                   (ui->ctrls[i].max - ui->ctrls[i].min)),
                      0, 1, 0);
            break;
        default:
...
static void onMotion(PuglView *view, int x, int y)
{
    ...
    { // menu
        if (MOUSEIN(MENU_SAVEP, fx, fy))
            ui->mouseover |= HOVER_MSAVEP;
        if (MOUSEIN(MENU_SAVEC, fx, fy))
            ui->mouseover |= HOVER_MSAVEC;
        if (MOUSEIN(MENU_LOAD, fx, fy))
            ui->mouseover |= HOVER_MLOAD;
        if (MOUSEIN(MENU_PGMS, fx, fy))
            ui->mouseover |= HOVER_MPGMS;
        if (MOUSEIN(MENU_PGML, fx, fy))
            ui->mouseover |= HOVER_MPGML;
        if (MOUSEIN(MENU_ACFG, fx, fy))
            ui->mouseover |= HOVER_MACFG;
    ...
    else if (ui->textentry_active || ui->popupmsg || ui->displaymode)
    {
        if (MOUSEIN(BTNLOC_OK, fx, fy))
            ui->mouseover |= HOVER_OK;
        if (MOUSEIN(BTNLOC_NO, fx, fy))
            ui->mouseover |= HOVER_NO;
        if (MOUSEIN(BTNLOC_YES, fx, fy))
            ui->mouseover |= HOVER_YES;
        if (MOUSEIN(BTNLOC_SAVE, fx, fy))
            ui->mouseover |= HOVER_SAVE;
        if (MOUSEIN(BTNLOC_DFLT, fx, fy))
            ui->mouseover |= HOVER_DFLT;
        if (MOUSEIN(BTNLOC_CANC, fx, fy))
            ui->mouseover |= HOVER_CANC;
        if (MOUSEIN(BTNLOC_CANC2, fx, fy))
            ui->mouseover |= HOVER_CANC2;
        if (MOUSEIN(BTNLOC_CANC3, fx, fy))
            ui->mouseover |= HOVER_CANC3;
        if (MOUSEIN(BTNLOC_RSRC, fx, fy))
            ui->mouseover |= HOVER_RSRC;
        if (MOUSEIN(SCROLLBAR, fx, fy))
            ui->mouseover |= HOVER_SCROLLBAR;
        if (phov != ui->mouseover)
        {
            puglPostRedisplay(view);
        }
    ...
    if (IS_FILEBROWSER(ui) && ui->dir_scrollgrab != NOSCROLL)
    {
        ...
        if (ui->dir_scroll < 0)
            ui->dir_scroll = 0;
        if (ui->dir_scroll > pages - 5)
            ui->dir_scroll = pages - 5;
...
static void onMouse(PuglView *view, int button, bool press, int x, int y)
{
    ...
    if (ui->popupmsg)
    {
        ...
        if (ui->pendingmode)
        {
            ...
            else if (MOUSEIN(BTNLOC_YES, fx, fy))
            { // YES
                handle_msg_reply(view);
            }
            else
    ...
    case 3:
        ...
        if (ui->pgm_sel >= 0)
        {
            txtentry_start(view, "Enter Preset Name:",
                           strlen(ui->midipgm[ui->pgm_sel]) > 0 ? ui->midipgm[ui->pgm_sel]
                                                                : "User");
            puglPostRedisplay(view);
        }
        else if (MOUSEIN(BTNLOC_CANC2, fx, fy))
    ...
    case 8:
        ...
        else
        {
            ...
            {
                cfg_update_value(view, cfg - 1,
                                 tri * ((puglGetModifiers(view) & PUGL_MOD_CTRL) ? 5 : 1));
            }
    ...
    case 6: // IS_FILEBROWSER() == displaymode 4,5,6
        ...
        else if (ui->dirlistlen > 120 && MOUSEIN(SCROLLBAR, fx, fy))
        {
            // handle scrollbar
            int pages = (ui->dirlistlen / 20);
            float ss = 1.6 / (float)pages;
            float sw = 5.0 * ss;
            float sx = ui->dir_scroll * ss - .8;
            if (fx < sx && ui->dir_scroll > 0)
                --ui->dir_scroll;
            else if (fx > sx + sw && ui->dir_scroll < (pages - 4))
                ++ui->dir_scroll;
        ...
        else if ((ui->displaymode == 5 || ui->displaymode == 6) && MOUSEIN(BTNLOC_SAVE, fx, fy))
        {
            txtentry_start(view, "Enter File Name:", "");
            return;
        }
    ...
    if (button == 1 && ui->displaymode == 0 && fx >= 1.04 && fx <= 1.100 && fy >= -.24 &&
        fy <= -.18 ANDNOANIM)
    {
        /* help button */
        reset_state_ccbind(view);
    ...
    if (button == 1 && ui->displaymode == 0 && fx >= 0.92 && fx <= 1.04 && fy >= -.25 &&
        fy <= -.16 ANDNOANIM)
    {
        /* menu button */
        reset_state_ccbind(view);
...
static int sb3_gui_setup(B3ui *ui, const LV2_Feature *const *features)
{
    ...
    /* Set up GL UI */
    ui->view = puglCreate(parent, "setBfree", ui->width, ui->height, ui->width, ui->height, true,
                          false, 0);
                                ...
                                const LV2_Feature *const *features)
{
    ...
    else if (getenv("HOME"))
    {
        ...
        sprintf(ui->defaultConfigFile, "%s/Library/Preferences/setBfree/default.cfg",
                getenv("HOME"));
        sprintf(ui->defaultProgrammeFile, "%s/Library/Preferences/setBfree/default.pgm",
                getenv("HOME"));
#else // linux, BSD, etc
                       ...
                       uint32_t format, const void *buffer)
{
    ...
    if (obj->body.otype == ui->uris.sb3_cfgkv)
    {
        ...
        if (2 == lv2_atom_object_get(obj, ui->uris.sb3_cckey, &key, ui->uris.sb3_ccval, &val, 0) &&
            key && val)
        {
...
static const LV2UI_Descriptor descriptor = {SB3_URI "#ui", instantiate, cleanup, port_event,
                                            extension_data};
