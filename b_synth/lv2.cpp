...
void initSynth(struct b_instance *inst, double rate)
{
    ...
    /* initAll() */
    initToneGenerator(inst->synth, inst->midicfg, rate, nullptr);
...
static void freeSynth(struct b_instance *inst)
{
    if (!inst)
        return;
...
static void mcc_cb(const char *fnname, const unsigned char chn, const unsigned char cc,
                             ...
                             const LV2_Feature *const *features)
{
    ...
    store(handle, b3s->uris.sb3_state, cfg, strlen(cfg) + 1, b3s->uris.atom_String,
                              ...
                              LV2_Worker_Respond_Handle handle, uint32_t size, const void *data)
{
    ...
    case CMD_SAVECFG:
        ...
        if (x)
        {
            fprintf(x, "# setBfree config file\n# modificaions on top of default "
                       "config\n");
            LOCALEGUARD_START;
...
static inline void postrun(B3S *b3s)
{
                              ...
                              const char *bundle_path, const LV2_Feature *const *features)
{
    ...
    else if (getenv("HOME"))
    {
        ...
        sprintf(defaultProgrammeFile, "%s/Library/Preferences/setBfree/default.pgm",
                getenv("HOME"));
#else // linux, BSD, etc
...
static void run(LV2_Handle instance, uint32_t n_samples)
{
    ...
    if (!warning_printed && capacity < 4096)
    {
        ...
        fprintf(stderr, "B3LV2: if your LV2 host allows one to configure a "
                        "buffersize use at least 4kBytes.\n");
    }
    ...
    if (b3s->midiin)
    {
        ...
        while (!lv2_atom_sequence_is_end(&(b3s->midiin)->body, (b3s->midiin)->atom.size, ev))
        {
            if (ev->body.type == b3s->uris.midi_MidiEvent)
            {
                ...
                if (written + BUFFER_SIZE_SAMPLES < ev->time.frames && ev->time.frames < n_samples)
                {
                ...
                parse_raw_midi_data(b3s->inst, (uint8_t *)(ev + 1), ev->body.size);
                     ...
                     ev->body.type == b3s->uris.atom_Object)
            {
                ...
                else if (obj->body.otype == b3s->uris.sb3_uimccset)
                {
                    ...
                    lv2_atom_object_get(obj, b3s->uris.sb3_cckey, &flags, b3s->uris.sb3_ccval, &cmd,
                                        0);
                    if (cmd && flags)
                    {
                        midi_uiassign_cc(b3s->inst->midicfg, (const char *)LV2_ATOM_BODY(cmd),
                                         ((LV2_Atom_Int *)flags)->body);
                        b3s->dirty = true;
                    }
                }
                ...
                else if (obj->body.otype == b3s->uris.sb3_midisavepgm)
                {
                    ...
                    const LV2_Atom *name = NULL;
                    lv2_atom_object_get(obj, b3s->uris.sb3_cckey, &pgm, b3s->uris.sb3_ccval, &name,
                                        0);
                    if (pgm && name)
                    {
                        saveProgramm(b3s->inst, (int)((LV2_Atom_Int *)pgm)->body,
                                     (char *)LV2_ATOM_BODY(name), 0);
                        b3s->update_pgm_now = 1;
                        if (b3s->midnam)
                        {
                            b3s->midnam->update(b3s->midnam->handle);
    ...
    if (keychanged)
    {
        ...
        lv2_atom_forge_vector(&b3s->forge, sizeof(unsigned int), b3s->uris.atom_Int, MAX_KEYS / 32,
                              b3s->active_keys);
        lv2_atom_forge_pop(&b3s->forge, &frame);
    }
    ...
    if (b3s->update_gui_now)
    {
        ...
        b3s->suspend_ui_msg = 0;
        forge_kvconfigmessage(&b3s->forge, &b3s->uris, b3s->uris.sb3_cfgkv, "lv2.info",
                              b3s->lv2nfo);
...
static char *mn_file(LV2_Handle instance)
{
    B3S *b3s = (B3S *)instance;
    char model[21];
...
static char *mn_model(LV2_Handle instance)
{
    B3S *b3s = (B3S *)instance;
...
static void mn_free(char *v) { free(v); }
...
static const LV2_Descriptor descriptor = {SB3_URI, instantiate, connect_port, activate,
