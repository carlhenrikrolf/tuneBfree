...
void ctl_biased()
{
    ...
    pushIndent();
    sprintf(buf, "float v = %g + ((%g - %g) * (u * u));", BIAS_LO, BIAS_HI, BIAS_LO);
    codeln(buf);
    ...
    pushIndent();
    codeln("struct b_preamp *pp = (struct b_preamp *) pa;");
    sprintf(buf, "pp->sagZgb = %g + ((%g - %g) * u);", SAG_ZGB_LO, SAG_ZGB_HI, SAG_ZGB_LO);
    codeln(buf);
    ...
    sprintf(buf, "pp->adwFb = %g;", ADWS_FB);
    codeln(buf);
    sprintf(buf, "pp->adwFb2 = %g + ((%g - %g) * (((float) uc) / 31.0));", ADWS_FB2, ADWS_FB,
            ADWS_FB2);
    codeln(buf);
    popIndent();
    codeln("} else {");
    pushIndent();
    sprintf(buf, "pp->adwFb = %g + ((%g - %g) * (((float) (uc - 32)) / 31.0));", ADWS_FB,
            ADWS_FB_MAX, ADWS_FB);
    codeln(buf);
    ...
    codeln(buf);
    sprintf(buf, "pp->adwFb2 = %g + ((%g - %g) * (((float) (uc - 64)) / 63.0));", ADWS_FB,
            ADWS_FB_MAX, ADWS_FB);
    codeln(buf);
    popIndent();
    codeln("}");
...
{
    sprintf(buf, "if (getConfigParameter_fr (\"%s\", cfg, &pp->adwFb, %g, %g));",
            "xov.ctl_biased_fb", 0.0, ADWS_FB_MAX);
    codeln(buf);

    sprintf(buf, "else if (getConfigParameter_fr (\"%s\", cfg, &pp->adwFb2, %g, %g));",
            "xov.ctl_biased_fb2", 0.0, ADWS_FB2_MAX);
    codeln(buf);

    sprintf(buf, "else if (getConfigParameter_f (\"%s\", cfg, &pp->sagFb));", "xov.ctl_sagtobias");
    codeln(buf);
...
{
    codeln("{\"xov.ctl_biased_fb\", CFG_FLOAT, \"0.5821\", \"This parameter "
           "behaves somewhat like "
           "an analogue tone control for bass mounted before the overdrive "
           "stage. Unity is "
           "somewhere around the value 0.6, lesser values takes away bass and "
           "lowers the volume "
           "while higher values gives more bass and more signal into the "
           "overdrive. Must be less "
           "than 1.0.\", INCOMPLETE_DOC},");
    codeln("{\"xov.ctl_biased_fb2\", CFG_FLOAT, \"0.999\", \"The fb2 parameter "
           "has the same function "
           "as fb1 but controls the signal after the overdrive stage. Together "
           "the two parameters are "
           "useful in that they can reduce the amount of bass going into the "
           "overdrive and then "
           "recover it on the other side. Must be less than 1.0.\", "
           "INCOMPLETE_DOC},");
    codeln("{\"xov.ctl_sagtobias\", CFG_FLOAT, \"0.1880\", \"This parameter is "
           "part of an attempt to "
           "recreate an artefact called 'power sag'. When a power amplifier is "
           "under heavy load the "
           "voltage drops and alters the operating parameters of the unit, "
           "usually towards more and "
           "other kinds of distortion. The sagfb parameter controls the rate of "
           "recovery from the sag "
           "effect when the load is lifted. Must be less than 1.0.\", "
           "INCOMPLETE_DOC},");
}
