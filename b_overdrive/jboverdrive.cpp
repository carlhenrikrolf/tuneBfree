...
int oscb_quit(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg,
              void *user_data)
{
...
#define OSC_CALLBACK(PARAM, FN)                                                                    \
    int oscb_##PARAM(const char *path, const char *types, lo_arg **argv, int argc, lo_message msg, \
                     void *user_data)                                                              \
    {                                                                                              \
        float v = argv[0]->f;                                                                      \
        if (v < 0 || v > 1.0)                                                                      \
...
{
    fprintf(status ? stderr : stdout, "%s - the B Preamp/Overdrive Emulator\n\n", name);
    fprintf(status ? stderr : stdout,
            "Utility to write audio peak data to stdout of file as plain text of "
            "JSON data.\n"
            "\n");
    ...
    fprintf(status ? stderr : stdout,
            ...
            "Examples:\n");
    fprintf(status ? stderr : stdout,
            "%s -i system:capture_1 -o system:playback_1 -p bias=.5 -p "
            "feedback=0.9\n",
            name);
    fprintf(status ? stderr : stdout,
            "\n"
...
int main(int argc, char **argv)
{
    ...
    struct option long_options[] = {
        {"help", no_argument, 0, 'h'},    {"input", no_argument, 0, 'i'},
        {"output", no_argument, 0, 'o'},  {"parameter", required_argument, 0, 'p'},
    ...
    while ((c = getopt_long(argc, argv, optstring, long_options, NULL)) != -1)
    {
        ...
        case 'V':
            printf("%s %s\n\n", "jboverdrive", VERSION);
            printf("Copyright (C) 2003-2004 Fredrik Kilander <fk@dsv.su.se>\n"
    ...
    j_output_port =
        jack_port_register(j_client, "out", JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0);
