...
void setOutputStream(FILE *fp) { outstr = fp; }
...
void preIpolMixer()
{
    ...
    codeln("static void mixFilterWeights (void *pa, const float * ipolDef, const "
           "float * aalDef) {");
    pushIndent();
    ...
    for (row = 0; row < XOVER_RATE; row++)
    {
        ...
        {
            sprintf(buf, "pp->wi[%d][%d] = mix[%d];", row, column, ipold[row].weightIndex[column]);
            codeln(buf);
        }
    }
...
void funcHeader(char *funcName)
{
    ...
    sprintf(buf,
            "float * %s (void *pa, const float * inBuf, float * outBuf, size_t "
            "buflen)",
            funcName);
    codeln(buf);
    codeln("{");
...
void funcBody(void (*transferdef)(char *, char *))
{
    ...
    if (generatePreFilter)
    {
        ...
        codeln("*(pp->xzp) = (Z0 * pp->pr_b0) + (pp->pr_b1 * pp->pr_z1) + "
               "(pp->pr_b2 * pp->pr_z2);");
...
void adapterPreamp(char *funcName)
{
    ...
    pushIndent();
    codeln("struct b_preamp *pp = (struct b_preamp *) calloc(1, sizeof(struct "
           "b_preamp));");
...
void legacyInit()
{
    ...
    if (generatePreFilter)
    {
        codeln("useMIDIControlFunction (m, \"xov.prefilter.hz\", setPreFilterHz, "
               "pa);");
        codeln("useMIDIControlFunction (m, \"xov.prefilter.q\", setPreFilterQ, pa);");
        codeln("useMIDIControlFunction(m, \"xov.prefilter.gain\", setPreFilterG, "
               "pa);");
    ...
    if (generatePostFilter)
    {
        codeln("useMIDIControlFunction (m, \"xov.postfilter.hz\", setPostFilterHz, "
               "pa);");
        codeln("useMIDIControlFunction (m, \"xov.postfilter.q\", setPostFilterQ, "
               "pa);");
        codeln("useMIDIControlFunction (m, \"xov.postfilter.gain\", "
               "setPostFilterG, pa);");
...
#ifdef ADWS_PRE_DIFF
    sprintf(buf, "useMIDIControlFunction (m, \"xov.ctl_biased_fb\", ctl_biased_fb, pa);");
    codeln(buf);
#endif /* ADWS_PRE_DIFF */

#ifdef ADWS_POST_DIFF
    sprintf(buf, "useMIDIControlFunction (m, \"xov.ctl_biased_fb2\", "
                 "ctl_biased_fb2, pa);");
    codeln(buf);
#endif /* ADWS_POST_DIFF */

#ifdef ADWS_GFB
    sprintf(buf, "useMIDIControlFunction (m, \"xov.ctl_biased_gfb\", "
                 "ctl_biased_gfb, pa);");
    codeln(buf);
#endif /* ADWS_GFB */

#ifdef SAG_EMULATION
    sprintf(buf, "useMIDIControlFunction (m, \"xov.ctl_sagtobias\", ctl_sagtoBias, pa);");
    codeln(buf);
#endif /* SAG_EMULATION */

#ifdef ADWS_FAT_CTRL
    sprintf(buf, "useMIDIControlFunction (m, \"overdrive.character\", "
                 "ctl_biased_fat, pa);");
    codeln(buf);
#endif /* ADWS_FAT_CTRL */
...
#ifdef OUTPUT_GAIN
    sprintf(buf, "useMIDIControlFunction (m, \"overdrive.outputgain\", "
                 "setOutputGain, pa);");
    codeln(buf);
#endif /* OUTPUT_GAIN */
...
#ifdef INPUT_COMPRESS
    codeln("useMIDIControlFunction (m, \"xov.compressor.threshold\", "
           "setIpcThreshold, pa);");
    codeln("useMIDIControlFunction (m, \"xov.compressor.attack\", setIpcAttack, "
           "pa);");
    codeln("useMIDIControlFunction (m, \"xov.compressor.release\", "
           "setIpcRelease, pa);");
#endif /* INPUT_COMPRESS */
    popIndent();
    codeln("}");
...
void legacyConfig()
{
    ...
    sprintf(buf, "if (getConfigParameter_f (\"%s\", cfg, &%s)) return 1;", "overdrive.inputgain",
            "pp->inputGain");
    codeln(buf);

    sprintf(buf, "else if (getConfigParameter_f (\"%s\", cfg, &%s)) return 1;",
            "overdrive.outputgain", "pp->outputGain");
    codeln(buf);

#ifdef ADWS_GFB
    sprintf(buf,
            "else if (getConfigParameter_f (\"%s\", cfg, &v)) { %s(pp, v); "
            "return 1; }",
...
void writeDocumentation()
{
    ...
    pushIndent();
    codeln("{\"overdrive.inputgain\", CFG_FLOAT, \"0.3567\", \"This is how much "
           "the input signal "
           "is scaled as it enters the overdrive effect. The default value is "
           "quite hot, but you "
           "can of course try it in anyway you like; range [0..1]\", "
           "INCOMPLETE_DOC},");
    codeln("{\"overdrive.outputgain\", CFG_FLOAT, \"0.07873\", \"This is how much "
           "the signal is "
           "scaled as it leaves the overdrive effect. Essentially this value should "
           "be as high as "
           "possible without clipping (and you *will* notice when it does - Test "
           "with a bass-chord "
           "on 88 8888 000 with percussion enabled and full swell, but do turn down "
           "the "
           "amplifier/headphone volume first!); range [0..1]\", INCOMPLETE_DOC},");
#ifdef TR_BIASED
    codeln("{\"xov.ctl_biased\", CFG_FLOAT, \"0.5347\", \"bias base; range "
           "[0..1]\", "
           "INCOMPLETE_DOC},");
#endif
#ifdef ADWS_GFB
    codeln("{\"xov.ctl_biased_gfb\", CFG_FLOAT, \"0.6214\", \"Global [negative] "
           "feedback control; "
           "range [0..1]\", INCOMPLETE_DOC},");
#endif
#ifdef CLEAN_MIX
    codeln("{\"xov...\", CFG_FLOAT, \"\", \"\", INCOMPLETE_DOC},"); /* TODO -
                                                                       unused in
                                                                       overmakerdefs.h
                                                                     */
#endif
    ...
    codeln("{\"overdrive.character\", CFG_FLOAT, \"-\", \"Abstraction to set "
           "xov.ctl_biased_fb and "
           "xov.ctl_biased_fb2\", INCOMPLETE_DOC},");
...
void preFilterControl()
{
    ...
    sprintf(buf, "pr_F = %g + ((%g - %g) * ((u * u) / 16129.0));", PRE_FILTER_HERTZ_LO,
            PRE_FILTER_HERTZ_HI, PRE_FILTER_HERTZ_LO);
    codeln(buf);
    sprintf(buf, "preFilterDefine (%d, pr_F, pr_Q, pr_G);", PRE_FILTER_TYPE);
    codeln(buf);
    ...
    pushIndent();
    sprintf(buf, "pr_Q = %g + ((%g - %g) * (((double) uc) / 127.0));", PRE_FILTER_Q_LO,
            PRE_FILTER_Q_HI, PRE_FILTER_Q_LO);
    codeln(buf);
    sprintf(buf, "preFilterDefine (%d, pr_F, pr_Q, pr_G);", PRE_FILTER_TYPE);
    codeln(buf);
    popIndent();
    ...
    pushIndent();
    sprintf(buf, "pr_G = %g + ((%g - %g) * (((double) uc) / 127.0));", PRE_FILTER_G_LO,
            PRE_FILTER_G_HI, PRE_FILTER_G_LO);
    codeln(buf);
    sprintf(buf, "preFilterDefine (%d, pr_F, pr_Q, pr_G);", PRE_FILTER_TYPE);
    codeln(buf);
    popIndent();
...
void postFilterControl()
{
    ...
    codeln("double u = (double) uc;");
    sprintf(buf, "de_F = %g + ((%g - %g) * ((u * u) / 16129.0));", POST_FILTER_HERTZ_LO,
            POST_FILTER_HERTZ_HI, POST_FILTER_HERTZ_LO);
    codeln(buf);
    sprintf(buf, "postFilterDefine(%d, de_F, de_Q, de_G);", POST_FILTER_TYPE);
    codeln(buf);
    ...
    pushIndent();
    sprintf(buf, "de_Q = %g + ((%g - %g) * (((double) uc) / 127.0));", POST_FILTER_Q_LO,
            POST_FILTER_Q_HI, POST_FILTER_Q_LO);
    codeln(buf);
    sprintf(buf, "postFilterDefine(%d, de_F, de_Q, de_G);", POST_FILTER_TYPE);
    codeln(buf);
    popIndent();
    ...
    pushIndent();
    sprintf(buf, "de_G = %g + ((%g - %g) * (((double) uc) / 127.0));", POST_FILTER_G_LO,
            POST_FILTER_G_HI, POST_FILTER_G_LO);
    codeln(buf);
    sprintf(buf, "postFilterDefine(%d, de_F, de_Q, de_G);", POST_FILTER_TYPE);
    codeln(buf);
    popIndent();
...
void outputGainControl()
{
    ...
    pushIndent();
    codeln("struct b_preamp *pp = (struct b_preamp *) pa;");
    sprintf(buf, "pp->outputGain = %g + ((%g - %g) * (((float) uc) / 127.0));", OUTPUT_GAIN_LO,
            OUTPUT_GAIN_HI, OUTPUT_GAIN_LO);
    codeln(buf);
...
void inputGainControl()
{
    ...
    pushIndent();
    codeln("struct b_preamp *pp = (struct b_preamp *) pa;");
    sprintf(buf, "pp->inputGain = %g + ((%g - %g) * (((float) uc) / 127.0));", INPUT_GAIN_LO,
            INPUT_GAIN_HI, INPUT_GAIN_LO);
    codeln(buf);
...
void inputCompressControl()
{
    ...
    sprintf(buf, "ipcThreshold = %g + ((%g - %g) * ratio);", IPC_THR_LO, IPC_THR_HI, IPC_THR_LO);
    codeln(buf);
    ...
    pushIndent();
    codeln("float ratio = ((float) uc) / 127.0;");
    sprintf(buf, "ipcGainReduce = %g + ((%g - %g) * ratio);", IPC_ATK_LO, IPC_ATK_HI, IPC_ATK_LO);
    codeln(buf);
    ...
    pushIndent();
    codeln("float ratio = ((float) uc) / 127.0;");
    sprintf(buf, "ipcGainRecover = %g + ((%g - %g) * ratio);", IPC_RLS_LO, IPC_RLS_HI, IPC_RLS_LO);
    codeln(buf);
