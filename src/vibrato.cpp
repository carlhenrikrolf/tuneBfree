 ...
 */
static void setVibrato(struct b_vibrato *v, int select)
{
    switch (select & 3)
    {
    case 0: /* disable */
...
static void initIncrementTables(struct b_vibrato *v)
{
...
#ifdef DEBUG_DUMP_SCANNER
    {
        ...
        {
            fprintf(fp, "statorIncrement = 0x%08x %g\n", v->statorIncrement,
 ...
 */
void reset_vibrato(struct b_vibrato *v)
{
    v->offsetTable = v->offset3Table;
    v->stator = 0;
    v->statorIncrement = 0;
...
int scannerConfig(void *t, ConfigContext *cfg)
{
    ...
    else if ((ack = getConfigParameter_dr("scanner.modulation.v2", cfg, &v->vib2OffAmp, 0.0,
                                          12.0)) == 1)
    {
    }
    else if ((ack = getConfigParameter_dr("scanner.modulation.v3", cfg, &v->vib3OffAmp, 0.0,
                                          12.0)) == 1)
    {
    }
    return ack;
 ...
 */
void vibratoProc(struct b_vibrato *v, float const *inbuffer, float *outbuffer,
...
static const ConfigDoc doc[] = {
    ...
    {"scanner.modulation.v1", CFG_DOUBLE, "3.0",
     "Amount of modulation for vibrato/chorus 1 setting", "Hz", 0, 12, .5},
    {"scanner.modulation.v2", CFG_DOUBLE, "6.0",
     "Amount of modulation for vibrato/chorus 2 setting", "Hz", 0, 12, .5},
    {"scanner.modulation.v3", CFG_DOUBLE, "9.0",
     "Amount of modulation for vibrato/chorus 3 setting", "Hz", 0, 12, .5},
    DOC_SENTINEL};

const ConfigDoc *scannerDoc() { return doc; }
