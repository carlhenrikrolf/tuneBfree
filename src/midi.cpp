...
static const char *ccFuncNames[] = {"upper.drawbar16",
                                    "upper.drawbar513",
                                    "upper.drawbar8",
                                    "upper.drawbar4",
                                    "upper.drawbar223",
                                    ...
                                    "rotary.speed-select", /* 0..8 (3^2 combinations)
...
const char *getCCFunctionName(int x) { return ccFuncNames[x]; }
 ...
 */
static void assignMIDIControllerFunction(ctrl_function *vec, unsigned char controller, int fn_id,
                                         void (*f)(void *, unsigned char), void *d)
{
    assert(vec != NULL);
    if (f != NULL)
    {
 ...
 */
void useMIDIControlFunction(void *mcfg, const char *cfname, void (*f)(void *, unsigned char),
                            void *d)
{
    struct b_midicfg *m = (struct b_midicfg *)mcfg;
...
void setControlFunctionCallback(void *mcfg,
                                void (*fn)(int, const char *, unsigned char, midiCCmap *, void *),
                                void *d)
{
    struct b_midicfg *m = (struct b_midicfg *)mcfg;
    m->hookfn = fn;
 ...
 */
void setKeyboardSplitMulti(void *mcfg, int flags, int p_splitA_PL, int p_splitA_UL, int p_nshA_PL,
                           int p_nshA_UL, int p_nshA_U)
{
    struct b_midicfg *m = (struct b_midicfg *)mcfg;
    if (flags & 1)
 ...
 */
static void loadCCMap(struct b_midicfg *m, const char *cfname, int ccn, unsigned char *A,
...
int midiConfig(void *mcfg, ConfigContext *cfg)
{
    ...
    int ack = 0;
    struct b_midicfg *m = (struct b_midicfg *)mcfg;
    if ((ack = getConfigParameter_ir("midi.upper.channel", cfg, &v, 1, 16)) == 1)
    {
        m->rcvChA = v - 1;
    }
    else if ((ack = getConfigParameter_ir("midi.lower.channel", cfg, &v, 1, 16)) == 1)
    {
        m->rcvChB = v - 1;
    }
    else if ((ack = getConfigParameter_ir("midi.pedals.channel", cfg, &v, 1, 16)) == 1)
    {
        m->rcvChC = v - 1;
    }
    else if ((ack = getConfigParameter_ir("midi.transpose", cfg, &v, -127, 127)) == 1)
    {
        m->transpose = v;
    }
    else if ((ack = getConfigParameter_ir("midi.upper.transpose", cfg, &v, -127, 127)) == 1)
    {
        m->nshA = v;
    }
    else if ((ack = getConfigParameter_ir("midi.lower.transpose", cfg, &v, -127, 127)) == 1)
    {
        m->nshB = v;
    }
    else if ((ack = getConfigParameter_ir("midi.pedals.transpose", cfg, &v, -127, 127)) == 1)
    {
        m->nshC = v;
    }
    else if ((ack = getConfigParameter_ir("midi.pedals.transpose.split", cfg, &v, -127, 127)) == 1)
    {
        m->nshA_PL = v;
    }
    else if ((ack = getConfigParameter_ir("midi.lower.transpose.split", cfg, &v, -127, 127)) == 1)
    {
        m->nshA_UL = v;
    }
    else if ((ack = getConfigParameter_ir("midi.upper.transpose.split", cfg, &v, -127, 127)) == 1)
    {
        m->nshA_U = v;
    }
    else if (strncasecmp(cfg->name, "midi.controller.reset", 21) == 0 && cfg->value &&
             strlen(cfg->value) > 0)
    {
        ack++;
        if (atoi(cfg->value))
            ...
            if (sscanf((cfg->name) + ccIdx, "%d", &ccn) == 1)
            {
                ...
                if ((0 <= ccn) && (ccn < 128))
                {
                    ...
                    else
                    {
                        ...
                        showConfigfileContext(cfg, "name of controllable function not found");
                    }
                }
                else
                {
...
void process_midi_event(void *instp, const struct bmidi_event_t *ev)
{
...
#ifdef DEBUG_MIDI_CC
    {
        ...
        printf("CC: %2d %03d -> %3d (%s) %s\n", ev->channel, ev->d.control.param,
               ev->d.control.value, fn,
               (m->ctrlvec[ev->channel] &&
                m->ctrlvec[ev->channel][ev->d.control.param].fn != emptyControlFunction)
                   ? "*"
                   : "-");
    }
#endif
        ...
        else
        {
            ...
            if (m->ccuimap >= 0)
            {
                ...
                memcpy(&m->ctrlvec[ev->channel][ev->d.control.param], &m->ctrlvecF[fnid],
                       sizeof(ctrl_function));
...
static void midi_print_cc_cb(const char *fnname, const unsigned char chn, const unsigned char cc,
                             const unsigned char flags, void *arg)
{
    FILE *fp = (FILE *)arg;
    fprintf(fp, "  %d  %03d  | %s %s\n", chn, cc, fnname, (flags & 1) ? "-" : "");
}

void midi_loopCCAssignment(void *mcfg, int ulp,
                           void (*cb)(const char *, unsigned char, unsigned char, unsigned char,
                                      void *),
                           void *arg)
{
    int i;
...
static const ConfigDoc doc[] = {
    {"midi.upper.channel", CFG_INT, "1",
     "The MIDI channel to use for the upper-manual. range: [1..16]", INCOMPLETE_DOC},
    {"midi.lower.channel", CFG_INT, "2",
     "The MIDI channel to use for the lower manual. range: [1..16]", INCOMPLETE_DOC},
    {"midi.pedals.channel", CFG_INT, "3", "The MIDI channel to use for the pedals. range: [1..16]",
     INCOMPLETE_DOC},
    {"midi.controller.reset", CFG_INT, "\"-\"",
     "Clear existing CC mapping for all controllers (if non-zero argument is "
     "given). See also -D "
     "option.",
     INCOMPLETE_DOC},
    {"midi.controller.upper.<cc>", CFG_TEXT, "\"-\"",
     "Specify a function-name to bind to the given MIDI control-command. <cc> "
     "is an integer "
     "0..127. Defaults are in midiPrimeControllerMapping() and can be listed "
     "using the '-d' "
     "commandline option. See general information.",
     INCOMPLETE_DOC},
    ...
    {"midi.transpose", CFG_INT, "0", "Global transpose (noteshift) in semitones.", "semitones",
     -127, 127, 1},
    {"midi.upper.transpose", CFG_INT, "0", "Shift/transpose MIDI-notes on upper-manual",
     "semitones", -127, 127, 1},
    {"midi.lower.transpose", CFG_INT, "0", "Shift/transpose MIDI-notes on lower-manual",
     "semitones", -127, 127, 1},
    {"midi.pedals.transpose", CFG_INT, "0", "Shift/transpose MIDI-notes on pedals", "semitones",
     -127, 127, 1},
