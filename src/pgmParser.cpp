...
{
    return ps->nextToken = getToken(ps->fp, &(ps->lineNumber), ps->stringBuffer, STRINGBUFFERSZ);
}

/*
 * Idempotent predicate: does the next token match the parameter?
 */
static int nextTokenMatches(ParserState *ps, B3TokenType t) { return ps->nextToken == t; }
 ...
 * the linenumber.
 */
static ParseReturnCode stateMessage(ParserState *ps, ParseReturnCode code, const char *msg)
{
    if (code == P_WARNING)
    {
