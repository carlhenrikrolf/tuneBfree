...
#ifdef CFG_MAIN
    printf("%s:%d:[%s]=[%s]\n", cfg->fname, cfg->linenr, cfg->name, cfg->value);
#else
    n += mainConfig(cfg);
...
void parseConfigurationLine(void *inst, const char *fname, int lineNumber, char *oneLine)
{
    ...
    if (*s == '=')
    {
        fprintf(stderr, "%s:line %d: empty parameter name.\n", fname, lineNumber);
        return;
    }
...
static void formatDoc(const char *modulename, const ConfigDoc *d)
{
    ...
    while (d && d->name)
    {
        ...
        printf("  %-40s   %s%s (%s)\n", d->name, conftypenames[d->type],
               (getCCFunctionId(d->name) < 0) ? " " : "*", (strlen(d->dflt) > 0) ? d->dflt : "?");
        if (strlen(d->desc) > 0)
        {
...
void dumpConfigDoc()
{
    printf("Instrument Properties:\n"
           ...
           "  'S': text-string, 'I': integer, 'F': float, 'D': double-precision "
           "float.\n"
           "  \n"
    ...
    printf("Note that the gain parameter does not apply to type 0 "
           "Low-Pass-Filters.\n\n");

    printf("Additional MIDI Control-Command Functions:\n"
           " These properties can not be modified directly, but are meant to be "
           "mapped\n"
    ...
    printf("  {upper|lower|pedal}.drawbar<NUM>           I* (-)\n"
           ...
           "    The range is inversely mapped to the position of the drawbar, so "
           "that fader-like "
           "controllers work in reverse, like real drawbars. Note that the MIDI "
           "controller values "
           "are quantized into 0 ... 8 to correspond to the nine discrete "
           "positions of the "
           "original drawbar system:\n"
           "    0:8 (loudest), 1-15:7, 16-31:6,  32-47:5, 48-63:4, 64-79:3, "
           "80-92:2, 96-110:1, "
           "111-127:0(off)\n"
...
void showConfigfileContext(ConfigContext *cfg, const char *msg)
{
    ...
    fprintf(stderr, "%s:line %d:name %s:value %s:%s\n", cfg->fname, cfg->linenr, cfg->name,
            cfg->value, msg);
}
...
void configDoubleOutOfRange(ConfigContext *cfg, double lowest, double highest)
{
    char buf[256];
    assert(cfg != NULL);
 ...
 */
const char *getConfigValue(ConfigContext *cfg) { return cfg->value; }
 ...
 *             -1 if the name matched but no assignment was made.
 */
int getConfigParameter_ir(const char *par, ConfigContext *cfg, int *ip, int lowInc, int highInc)
{
    int k;
    int rtn;
    ...
    if (rtn == 1)
    {
        ...
        {
            fprintf(stderr, "Value for config parameter %s is out range (%d--%d).", cfg->name,
                    lowInc, highInc);
            rtn = -1;
        }
    }
 ...
 *             -1 if the name matched but no assignment was made.
 */
int getConfigParameter_dr(const char *par, ConfigContext *cfg, double *dp, double lowInc,
                          double highInc)
{
    double d;
    int rtn;
 ...
 *             -1 if the name matched but no assignment was made.
 */
int getConfigParameter_fr(const char *par, ConfigContext *cfg, float *fp, float lowInc,
                          float highInc)
{
    double d;
    int rtn;
