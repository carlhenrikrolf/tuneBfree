...
static float FloatClamp01(float x) { return x >= 1.0f ? 1.0f : x <= 0.0f ? 0.0f : x; }
...
static void PluginProcessEvent(MyPlugin *plugin, const clap_event_header_t *event)
{
    if (event->space_id == CLAP_CORE_EVENT_SPACE_ID)
    {
        ...
        else if (event->type == CLAP_EVENT_PARAM_VALUE)
        {
            ...
            setDrawBar(plugin->synth, i, rint(valueEvent->value));
                           ...
                           float *outputL, float *outputR)
{
    while (written < nframes)
    {
        ...
        if (plugin->boffset >= BUFFER_SIZE_SAMPLES)
        {
            ...
            // plugin->bufD[0],
            //           plugin->bufD[1], BUFFER_SIZE_SAMPLES);
        }

        int nread = MIN(nremain, (BUFFER_SIZE_SAMPLES - plugin->boffset));

        // memcpy(&out[0][written], &plugin->bufL[0][plugin->boffset], nread * sizeof(float));
        // memcpy(&out[1][written], &plugin->bufL[1][plugin->boffset], nread * sizeof(float));
        memcpy(&outputL[written], &plugin->bufA[plugin->boffset], nread * sizeof(float));
        memcpy(&outputR[written], &plugin->bufA[plugin->boffset], nread * sizeof(float));
