...
static const char *portnames[AUDIO_CHANNELS] = {"out_left", "out_right"};
...
static int jack_audio_callback(jack_nframes_t nframes, void *arg)
{
    ...
    if (jack_midi_output && nframes > 0)
    {
        ...
        {
            jack_midi_event_write(jack_midi_outbuf, nframes - 1,
...
static int open_jack(void)
{
    ...
    j_output_bufferptrs = (jack_default_audio_sample_t **)calloc(
        AUDIO_CHANNELS, sizeof(jack_default_audio_sample_t *));
...
#endif
        j_output_port[i] =
            jack_port_register(j_client, channelid, JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0);
        if (!j_output_port[i])
        {
    ...
    { // use JACK-midi
        jack_midi_port =
            jack_port_register(j_client, "midi_in", JACK_DEFAULT_MIDI_TYPE, JackPortIsInput, 0);
        ...
        jack_midi_output =
            jack_port_register(j_client, "midi_out", JACK_DEFAULT_MIDI_TYPE, JackPortIsOutput, 0);
    }
    ...
    jack_srate_callback(jack_get_sample_rate(j_client),
                        NULL); // force geting samplerate
...
static void connect_jack_ports()
{
    ...
    for (i = 0; i < AUDIO_CHANNELS; i++)
    {
        ...
        {
            fprintf(stderr, "cannot connect port %s to %s\n", jack_port_name(j_output_port[i]),
                    jack_port[i]);
        }
        c++;
    }
    ...
    if (c == 0 && jack_ports && strlen(jack_ports) > 0)
    {
        ...
        const char **found_ports =
            jack_get_ports(j_client, jack_ports, JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput);
        if (found_ports)
        {
    ...
    {
        const char **found_ports =
            jack_get_ports(j_client, midi_port, JACK_DEFAULT_MIDI_TYPE, JackPortIsOutput);
        if (found_ports)
        {
...
static void initAll()
{
    ...
    initConvolution(NULL, inst.midicfg, AUDIO_CHANNELS, BUFFER_SIZE_SAMPLES, s_param.sched_priority,
                    s_policy);
#endif
...
static void Usage(int configdoc)
{
    ...
    printf("%s - DSP tonewheel organ\n\n"
           ...
           name);
    printf("Usage: %s [ OPTIONS ] [ property=value ... ]\n\n", name);
    printf("Options:\n"
           ...
           "                    (may override previous settings given with -r, "
           "-U,..)\n"
    ...
    if (configdoc)
    {
        printf("General Information:\n"
               ...
               "configurable\n"
               "  parameters have built-in default values. They can be overridden on "
               "startup\n"
               "  using 'property=value' pairs or by loading a specific .cfg file.\n"
               "  As the name /static/ implies, the properties can only be set on "
               "application\n"
               "  start. Yet many of the properties merely define the initial value "
               "of\n"
               "  settings which can later be modified during playback. The ones "
               "which can be\n"
               "  dynamically modified are marked with an asterisk (*)\n"
               "  \n"
               "  Properties are modified by sending MIDI Control-Commands (CC) to "
               "the synth.\n"
               ...
               "  (Note: each function can assigned only once, however MIDI-CC can be "
               "re-used\n"
               "  and trigger multiple function at the same time.)\n"
               "  \n"
               "  The program basically defines 'shortcuts'. Loading a program is "
               "usually\n"
               "  equivalent to sending a series of CC. Programs are commonly used to "
               "define\n"
               "  instruments (e.g. draw-bar settings to mimic a flute) or provide "
               "scale-\n"
               "  points (e.g. reverb=64).  There are a few special commands which "
               "are\n"
               "  only available by recalling a program (randomize settings, "
               "split-manuals,\n"
               "  enable overdrive).\n"
               "  \n"
               ...
               "  messages (also known as 'presets') MIDI-banks are ignored. So at "
               "most\n"
               "  127 programs can be specified.\n"
               "\n"
               "  At startup 'default.cfg' and 'default.pgm' in "
               "$XDG_CONFIG_HOME/setBfree/\n"
               "  (default: $HOME/.config/setBfree/) are are evaluated if the files "
               "exist,\n"
               "  unless '--noconfig' or '--noprogram' options are given.\n"
               "  An additional config or program file can be loaded using the '-c' "
               "and '-p'\n"
               "  option respectively.\n"
               "\n");
        printf("  Example config: \"%s\"\n\n  Example program: \"%s\"\n\n", templateConfigFile,
               templateProgrammeFile);
        dumpConfigDoc();
    }
    printf("Examples:\n");
    printf("%s\n", name);
    printf("%s -p pgm/default.pgm midi.port=\"a2j:[AV]\" midi.driver=jack\n", name);
    printf("%s midi.port=129 midi.driver=alsa jack.connect=jack_rack:in_\n", name);
    printf("%s jack.out.left=system:playback_7 jack.out.right=system:playback_8\n", name);
    printf("%s -U 868000000\n", name);
    printf("\n"
...
static void PrintVersion()
{
    const char *name = "setBfree";
    printf("%s %s\n\n", name, VERSION);
    printf("Copyright (C) 2008-2018 Robin Gareus <robin@gareus.org>\n"
           "Copyright (C) 2012 Will Panther <pantherb@setbfree.org>\n"
...
static const ConfigDoc doc[] = {
    {"midi.driver", CFG_TEXT, "\"jack\"", "The midi driver to use, 'jack' or 'alsa'",
     INCOMPLETE_DOC},
    {"midi.port", CFG_TEXT, "\"\"",
     "The midi port(s) to auto-connect to. With alsa it's a single port-name "
     "or number, jack "
     "accepts regular expressions.",
     INCOMPLETE_DOC},
    {"jack.connect", CFG_TEXT, "\"system:playback_\"",
     "Auto connect both audio-ports to a given regular-expression. This "
     "setting is ignored if "
     "either of jack.out.{left|right} is specified.",
     INCOMPLETE_DOC},
...
const ConfigDoc *mainDoc() { return doc; }
...
int main(int argc, char *argv[])
{
    ...
    const struct option long_options[] = {
        {"help", no_argument, 0, 'H'},         {"program", required_argument, 0, 'p'},
        {"config", required_argument, 0, 'c'}, {"noconfig", no_argument, 0, 'C'},
        {"dumpcc", no_argument, 0, 'd'},       {"noCC", no_argument, 0, 'D'},
        {"midnam", required_argument, 0, 'M'}, {"noprogram", no_argument, 0, 'P'},
        {"randomize", no_argument, 0, 'r'},    {"upper", required_argument, 0, 'U'},
    ...
    while ((c = getopt_long(argc, argv, optstring, long_options, NULL)) != -1)
    {
        ...
        case 'r':
            ...
            fprintf(stderr,
                    "Random Preset: "
                    "%d%d %d%d%d%d %d%d%d\n",
                    randomPreset[0], randomPreset[1], randomPreset[2], randomPreset[3],
                    randomPreset[4], randomPreset[5], randomPreset[6], randomPreset[7],
                    randomPreset[8]);
    ...
    {
        parseConfigurationLine(&inst, "commandline argument", 0, configOverride[i]);
    }
 ...
 */
  ...
  setDrawBars (&inst, 2, presetSelect); /* 86 - */
    ...
    while (j_client) /* jack callback is doing this the work now */
#ifdef _WIN32
        Sleep(1000);
#else
        sleep(1);
