...
{
    fprintf(f, "  </MasterDeviceNames>\n"
               "</MIDINameDocument>");
}

static void midnam_print_cc_cb(const char *fnname, const unsigned char chn, const unsigned char cc,
...
static void midnam_print_pgm_cb(int num, int pc, const char *name, void *arg)
{
    FILE *fp = (FILE *)arg;
...
static void midnam_avail_channel(FILE *f, int c)
{
    int i;
    for (i = 1; i <= 16; i++)
    {
...
static void midnam_channe_set(FILE *f, const char *name, const char *ctrl, int c)
{
    fprintf(f,
...
void save_midname(void *instp, FILE *f, char *model)
{
    ...
    fprintf(f,
            ...
            "        <ChannelNameSetAssign Channel=\"%d\" NameSet=\"Upper "
            "Manual\"/>\n"
            "        <ChannelNameSetAssign Channel=\"%d\" NameSet=\"Lower "
            "Manual\"/>\n"
