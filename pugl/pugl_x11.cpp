...
*/
static int attrListSgl[] = {GLX_RGBA, GLX_RED_SIZE,   4,  GLX_GREEN_SIZE,      4, GLX_BLUE_SIZE,
                            4,        GLX_DEPTH_SIZE, 16, GLX_ARB_multisample, 1, None};
...
/**
   Attributes for double-buffered RGBA with multi-sampling
         (antialiasing)
*/
static int attrListDblMS[] = {GLX_RGBA, GLX_DOUBLEBUFFER,   True, GLX_RED_SIZE,   4, GLX_GREEN_SIZE,
                              4,        GLX_BLUE_SIZE,      4,    GLX_ALPHA_SIZE, 4, GLX_DEPTH_SIZE,
                              16,       GLX_SAMPLE_BUFFERS, 1,    GLX_SAMPLES,    4, None};

PuglView *puglCreate(PuglNativeWindow parent, const char *title, int min_width, int min_height,
                     int width, int height, bool resizable, bool ontop, unsigned long transientId)
{
...
#ifdef VERBOSE_PUGL
        printf("puGL: singlebuffered rendering will be used, no doublebuffering "
               "available\n");
#endif
    }
    ...
    Window xParent = parent ? (Window)parent : RootWindow(impl->display, impl->screen);

    Colormap cmap = XCreateColormap(impl->display, xParent, vi->visual, AllocNone);
    ...
    impl->win =
        XCreateWindow(impl->display, xParent, 0, 0, view->width, view->height, 0, vi->depth,
    ...
    { /* TODO stay on top  */
        ...
        XChangeProperty(impl->display, impl->win,
                        ...
                        XInternAtom(impl->display, "ATOM", False), 32, PropModeReplace,
...
void puglDestroy(PuglView *view)
{
    if (!view)
    {
        return;
...
PUGL_API void puglShowWindow(PuglView *view) { XMapRaised(view->impl->display, view->impl->win); }

PUGL_API void puglHideWindow(PuglView *view) { XUnmapWindow(view->impl->display, view->impl->win); }

static void puglReshape(PuglView *view, int width, int height)
{
...
static void puglDisplay(PuglView *view)
{
    glXMakeCurrent(view->impl->display, view->impl->win, view->impl->ctx);
#if 0
...
static void puglResize(PuglView *view)
{
    int set_hints = 1;
    view->resize = false;
    if (!view->resizeFunc)
...
static PuglKey keySymToSpecial(KeySym sym)
{
    switch (sym)
    {
    case XK_F1:
...
static void setModifiers(PuglView *view, unsigned xstate, unsigned xtime)
{
...
PuglStatus puglProcessEvents(PuglView *view)
{
    ...
    while (XPending(view->impl->display) > 0)
    {
        ...
        case ConfigureNotify:
            ...
            {
                puglReshape(view, event.xconfigure.width, event.xconfigure.height);
            }
            break;
        case Expose:
        ...
        case ButtonRelease:
            setModifiers(view, event.xbutton.state, event.xbutton.time);
            if (view->mouseFunc && (event.xbutton.button < 4 || event.xbutton.button > 7))
            {
                view->mouseFunc(view, event.xbutton.button, event.type == ButtonPress,
        ...
        case KeyRelease:
        {
            setModifiers(view, event.xkey.state, event.xkey.time);
            bool repeated = false;
            if (view->ignoreKeyRepeat && XEventsQueued(view->impl->display, QueuedAfterReading))
            {
                XEvent next;
        ...
        case ClientMessage:
        {
            char *type = XGetAtomName(view->impl->display, event.xclient.message_type);
...
void puglPostRedisplay(PuglView *view) { view->redisplay = true; }

void puglPostResize(PuglView *view) { view->resize = true; }

PuglNativeWindow puglGetNativeWindow(PuglView *view) { return view->impl->win; }

int puglOpenFileDialog(PuglView *view, const char *title)
{
#ifdef WITH_SOFD
...
int puglUpdateGeometryConstraints(PuglView *view, int min_width, int min_height, bool aspect)
{
