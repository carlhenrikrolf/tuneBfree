                     ...
                     int width, int height, bool resizable, bool ontop, unsigned long transientId)
{
    ...
    {
        view->impl->win_flags =
            WS_POPUPWINDOW | WS_CAPTION | (view->user_resizable ? WS_SIZEBOX : 0);
    }
    ...
    impl->hwnd = CreateWindowEx(
        WS_EX_TOPMOST, classNameBuf, title,
        (view->user_resizable ? WS_SIZEBOX : 0) |
            (parent ? (WS_CHILD | WS_VISIBLE) : (WS_POPUPWINDOW | WS_CAPTION)),
        0, 0, wr.right - wr.left, wr.bottom - wr.top, (HWND)parent, NULL, NULL, NULL);
    ...
    SetWindowPos(impl->hwnd, ontop ? HWND_TOPMOST : HWND_TOP, 0, 0, 0, 0,
                 (ontop ? 0 : SWP_NOACTIVATE) | SWP_ASYNCWINDOWPOS | SWP_NOMOVE | SWP_NOSIZE);
...
PUGL_API void puglShowWindow(PuglView *view)
{
...
PUGL_API void puglHideWindow(PuglView *view)
{
...
static void puglResize(PuglView *view)
{
    ...
    if (parent)
    {
        ...
        SetWindowPos(view->impl->hwnd, HWND_TOP, 0, 0, view->width, view->height,
    ...
    AdjustWindowRectEx(&wr, view->impl->win_flags, FALSE, WS_EX_TOPMOST);
    SetWindowPos(view->impl->hwnd, view->ontop ? HWND_TOPMOST : HWND_NOTOPMOST, 0, 0,
...
static PuglKey keySymToSpecial(int sym)
{
    switch (sym)
    {
    case VK_F1:
...
static void processMouseEvent(PuglView *view, int button, bool press, LPARAM lParam)
{
    ...
    if (view->mouseFunc)
    {
        view->mouseFunc(view, button, press, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
    }
}

static void setModifiers(PuglView *view)
{
    view->mods = 0;
...
static LRESULT handleMessage(PuglView *view, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEWHEEL:
        if (view->scrollFunc)
        {
            ...
            view->scrollFunc(view, pt.x, pt.y, 0,
    ...
    case WM_MOUSEHWHEEL:
        if (view->scrollFunc)
        {
            ...
            POINT pt = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
            ScreenToClient(view->impl->hwnd, &pt);
            view->scrollFunc(view, pt.x, pt.y, GET_WHEEL_DELTA_WPARAM(wParam) / (float)WHEEL_DELTA,
                             0);
        }
        break;
    case WM_KEYDOWN:
...
PuglNativeWindow puglGetNativeWindow(PuglView *view) { return (PuglNativeWindow)view->impl->hwnd; }
...
int puglOpenFileDialog(PuglView *view, const char *title)
{
    ...
    ofn.Flags = OFN_ENABLESIZING | OFN_FILEMUSTEXIST | OFN_NONETWORKBUTTON | OFN_HIDEREADONLY |
                OFN_READONLY;

    // TODO look into async ofn.lpfnHook, OFN_ENABLEHOOK
    // UINT_PTR CALLBACK openFileHook(HWND hDlg, UINT message, WPARAM wParam,
    // LPARAM lParam) yet it seems GetOpenFIleName itself won't return anyway.
...
int puglUpdateGeometryConstraints(PuglView *view, int min_width, int min_height, bool aspect)
{
    ...
    if (view->width < min_width || view->height < min_height)
    {
        if (!view->resize)
        {
            ...
            view->resize = true;
            // TODO: trigger resize
            // (not always needed since this fn is usually called in response to a
            // resize)
        }
    }
    return 0;
